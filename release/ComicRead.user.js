...
`;break;default:o=GM_getResourceText(t.replaceAll("/","|"))}if(!o)throw new Error(`\u5916\u90E8\u6A21\u5757 ${t} \u672A\u5728 @Resource \u4E2D\u58F0\u660E`);if(t.startsWith("worker/")&&supportWorker)try{const a=`
const exports = {};
const Comlink = require('comlink');
${o}
Comlink.expose(exports);
`.replaceAll(/const (\w+?) = require\('(.+?)'\);/g,(f,h,c)=>`
let ${h} = {};
(function (exports, module) { ${GM_getResourceText(c)} }) (
  ${h},
  {
    set exports(value) { ${h} = value },
    get exports() { return ${h} }
  }
);`),n=URL.createObjectURL(new Blob([a],{type:"text/javascript"}));setTimeout(URL.revokeObjectURL,0,n);const l=new Worker(n);unsafeWindow[tempName][t]=require("comlink").wrap(l);return}catch{supportWorker=!1}let r=`
    (function (process, require, exports, module, ${gmApiList.join(", ")}) {
      ...
      ${gmApiList.map(a=>`window['${tempName}'].${a}`).join(", ")}
    );
  `;unsafeWindow[tempName]=crsLib,unsafeWindow[tempName][t]={},evalCode(r),Reflect.deleteProperty(unsafeWindow,tempName)},require=t=>{const o={value:!0},r=()=>{};r.default={};const a=new Proxy(r,{get(n,l){if(l==="__esModule")return o;if(l==="default")return a;crsLib[t]||selfImportSync(t);const f=crsLib[t];return Reflect.has(crsLib[t],"default")&&Reflect.has(crsLib[t].default,l)?f.default?.[l]:f?.[l]},apply(n,l,f){crsLib[t]||selfImportSync(t);const h=crsLib[t];return(typeof h.default=="function"?h.default:h)(...f)},construct(n,l){crsLib[t]||selfImportSync(t);const f=crsLib[t],h=typeof f.default=="function"?f.default:f;return new h(...l)}});return a};crsLib.require=require;const languages=require("helper/languages"),otherSite=require("userscript/otherSite"),helper=require("helper"),main=require("main");let options;try{switch(window.location.hostname){case"bbs.yamibo.com":{const t=require("solid-js/web"),o=require("solid-js"),r=require("main"),a=require("helper");(async()=>{const{options:n,setComicLoad:l,showComic:f,loadComic:h,setManga:c,needAutoShow:d}=await r.useInit("yamibo",{\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6:!0,\u5173\u95ED\u5FEB\u6377\u5BFC\u822A\u7684\u8DF3\u8F6C:!0,\u4FEE\u6B63\u70B9\u51FB\u9875\u6570\u65F6\u7684\u8DF3\u8F6C\u5224\u5B9A:!0,\u56FA\u5B9A\u5BFC\u822A\u6761:!0,\u81EA\u52A8\u7B7E\u5230:!0});if(GM_addStyle(`#fab { --fab: #6E2B19; }
    ...
    `),unsafeWindow.discuz_uid&&unsafeWindow.discuz_uid!=="0"&&n.\u81EA\u52A8\u7B7E\u5230&&(async()=>{const u=new Date().toLocaleDateString("zh-CN");if(u===localStorage.getItem("signDate"))return;const m=a.querySelector('#scbar_form > input[name="formhash"]')?.value;if(m)try{const b=await(await fetch(`plugin.php?id=zqlj_sign&sign=${m}`)).text();if(!/成功！|打过卡/.test(b))throw new Error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25");r.toast.success("\u81EA\u52A8\u7B7E\u5230\u6210\u529F"),localStorage.setItem("signDate",u)}catch{r.toast.error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25")}})(),n.\u5173\u95ED\u5FEB\u6377\u5BFC\u822A\u7684\u8DF3\u8F6C&&a.querySelector("#qmenu a")?.setAttribute("href","javascript:;"),/thread(-\d+){3}|mod=viewthread/.test(document.URL)){for(const m of a.querySelectorAll('img[file*="sinaimg.cn"]'))m.setAttribute("referrerpolicy","no-referrer");const u=unsafeWindow.fid??Number(new URLSearchParams(a.querySelector("h2 > a")?.href).get("fid")??"-1");if(u===30||u===37){!a.querySelector(".pg > .prev")||(d.val=!1);let g=a.querySelectorAll(":is(.t_fsz, .message) img");const b=()=>{let p=g.length;for(;p--;){const s=g[p],y=s.getAttribute("file");y&&s.src!==y&&(s.setAttribute("src",y),s.setAttribute("lazyloaded","true")),(s.src.includes("static/image")||s.complete&&s.naturalHeight&&s.naturalWidth&&s.naturalHeight<500&&s.naturalWidth<500)&&g.splice(p,1)}return g.map(s=>s.src)};l(b),c({onLoading(p,s){if(s&&s.width<500&&s.height<500)return h()},onExit(p){p&&a.scrollIntoView(".psth, .rate, #postlist > div:nth-of-type(2)"),c("show",!1)}}),a.querySelector("div.pti > div.authi")&&(a.querySelector("div.pti > div.authi").insertAdjacentHTML("beforeend",'<span class="pipe show">|</span><a id="comicReadMode" class="show" href="javascript:;">\u6F2B\u753B\u9605\u8BFB</a>'),document.getElementById("comicReadMode")?.addEventListener("click",()=>f())),a.querySelector("#threadindex")&&a.hijackFn("ajaxinnerhtml",()=>{g=a.querySelectorAll(".t_fsz img"),!(g.length===0||b().length===0)&&n.autoShow&&f()});const w=a.querySelector(".ptg.mbm.mtn > a");if(w){const p=w.href.split("id=")[1],s=/(?<=<th>\s<a href="thread-)\d+(?=-)/g;let y=[];const _=async($=1)=>{const L=[...(await r.request(`/misc.php?mod=tag&id=${p}&type=thread&page=${$}`)).responseText.matchAll(s)].map(([x])=>Number(x));y=y.concat(L);const M=y.indexOf(unsafeWindow.tid);return L.length>0&&(M===-1||!y[M+1])?_($+1):c({onPrev:y[M-1]?()=>window.location.assign(`thread-${y[M-1]}-1-1.html`):void 0,onNext:y[M+1]?()=>window.location.assign(`thread-${y[M+1]}-1-1.html`):void 0})};setTimeout(_)}}if(n.\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6){const m=unsafeWindow.tid??new URLSearchParams(window.location.search).get("tid")??/\/thread-(\d+)-\d+-\d+.html/.exec(window.location.pathname)?.[1];if(!m)return;let g;try{const I=await r.request(`/api/mobile/index.php?module=viewthread&tid=${m}`,{responseType:"json",errorText:"\u83B7\u53D6\u5E16\u5B50\u56DE\u590D\u6570\u65F6\u51FA\u9519",noTip:!0});g=Number.parseInt(I.response?.Variables?.thread?.allreplies,10)}catch{}const b=Number.parseInt(a.querySelector("#pgt strong")?.textContent??a.querySelector("#dumppage")?.value??"1",10),w=await a.useCache({history:"tid"}),p=await w.get("history",`${m}`);if(p&&b<p.lastPageNum)return;const s=a.querySelectorAll(p?.lastAnchor&&b===p.lastPageNum?`#${p.lastAnchor} ~ div`:"#postlist > div, .plc.cl");if(s.length===0)return;let y=0;const _=I=>{y&&window.clearTimeout(y),y=window.setTimeout(async()=>{y=0,await w.set("history",I)},200)},$=new IntersectionObserver(I=>{const L=I.find(x=>x.isIntersecting);if(!L)return;const M=s.indexOf(L.target);if(M!==-1){for(const x of s.splice(0,M+1))$.unobserve(x);_({tid:`${m}`,lastPageNum:b,lastReplies:g||p?.lastReplies||0,lastAnchor:L.target.id})}},{rootMargin:"-160px"});for(const I of s)$.observe(I)}return}if(/forum(-\d+){2}|mod=forumdisplay/.test(document.URL)){if(n.\u4FEE\u6B63\u70B9\u51FB\u9875\u6570\u65F6\u7684\u8DF3\u8F6C\u5224\u5B9A){const u=a.querySelectorAll(".tps>a");let m=u.length;for(;m--;)u[m].setAttribute("onClick","atarget(this)")}if(n.\u8BB0\u5F55\u9605\u8BFB\u8FDB\u5EA6){const u=await a.useCache({history:"tid"}),m=!document.querySelector("#flk"),[g,b]=o.createSignal(!1),w=()=>b(_=>!_);let p="tbody[id^=normalthread]",s=_=>_.id.split("_")[1],y=(_,$)=>`thread-${$}-${_.lastPageNum}-1.html#${_.lastAnchor}`;m&&(p=".threadlist li.list",s=_=>new URLSearchParams(_.children[1].getAttribute("href")).get("tid"),y=(_,$)=>`forum.php?mod=viewthread&tid=${$}&extra=page%3D1&mobile=2&page=${_.lastPageNum}#${_.lastAnchor}`);for(const _ of a.querySelectorAll(p)){const $=s(_);t.render(()=>{const[I,L]=o.createSignal();a.createEffectOn(g,()=>u.get("history",$).then(L));const M=o.createMemo(()=>I()?y(I(),$):""),x=o.createMemo(()=>!m&&I()?Number(_.querySelector(".num a").innerHTML)-I().lastReplies:0),A=()=>[(()=>{var S=t.template("<a class=historyTag>\u56DE\u7B2C<!>\u9875 ")(),T=S.firstChild,R=T.nextSibling;return R.nextSibling,t.addEventListener(S,"click",unsafeWindow.atarget,!0),t.insert(S,()=>I()?.lastPageNum,R),t.effect(()=>t.setAttribute(S,"href",M())),S})(),t.createComponent(o.Show,{get when(){return x()>0},get children(){var S=t.template("<div class=historyTag>+")();return S.firstChild,t.insert(S,x,null),S}})],v=()=>(()=>{var S=t.template("<li><a>\u56DE\u7B2C<!>\u9875")(),T=S.firstChild,R=T.firstChild,E=R.nextSibling;return E.nextSibling,t.addEventListener(T,"click",unsafeWindow.atarget,!0),T.style.setProperty("color","unset"),t.insert(T,()=>I()?.lastPageNum,E),t.effect(()=>t.setAttribute(T,"href",M())),S})();return t.createComponent(o.Show,{get when(){return!!I()},get children(){return t.createComponent(o.Show,{when:m,get children(){return v()},get fallback(){return A()}})}})},m?_.children[3]:_.getElementsByTagName("th")[0])}document.addEventListener("visibilitychange",w),a.querySelector("#autopbn")?.addEventListener("click",w)}}})().catch(n=>a.log.error(n)),t.delegateEvents(["click"]);break}case"www.yamibo.com":{if(!window.location.pathname.includes("/manga/view-chapter"))break;const t=new URLSearchParams(window.location.search).get("id");if(!t)break;const o=Number(helper.querySelector("section div:first-of-type div:last-of-type").innerHTML.split("\uFF1A")[1]);if(Number.isNaN(o))throw new Error(helper.t("site.changed_load_failed"));const r=async n=>{const l=await main.request(`https://www.yamibo.com/manga/view-chapter?id=${t}&page=${n}`);return/(?<=<img id=['"]imgPic['"].+?src=['"]).+?(?=['"])/.exec(l.responseText)[0].replaceAll("&amp;","&").replaceAll("http://","https://")},a=n=>helper.plimit(helper.createSequence(o).map(l=>async()=>n(l,await r(l+1))));options={name:"newYamibo",getImgList:({dynamicLoad:n})=>n(a,o)(),onNext:helper.querySelectorClick("#btnNext"),onPrev:helper.querySelectorClick("#btnPrev"),onExit:n=>n&&helper.scrollIntoView("#w1")};break}case"comic.idmzj.com":case"comic.dmzj.com":case"manhua.idmzj.com":case"manhua.dmzj.com":{const t=require("solid-js/web"),o=require("solid-js"),r=require("main"),a=require("userscript/dmzjApi"),n=require("helper");(async()=>{const l=async()=>{const[,m,g]=window.location.pathname.split(/\/|\./);return m||r.toast.error("\u6F2B\u753B\u6570\u636E\u83B7\u53D6\u5931\u8D25",{duration:Number.POSITIVE_INFINITY,throw:new Error("\u83B7\u53D6\u6F2B\u753B\u62FC\u97F3\u7B80\u79F0\u5931\u8D25")}),{comicId:await a.getComicId(m),chapterId:g}},f=async()=>{if(await n.waitDom(".commentBox"),!n.querySelector(".cartoon_online_border > img"))return!1;n.querySelector(".cartoon_online_border").innerHTML="\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u4E2D";for(const g of n.querySelectorAll(".odd_anim_title ~ *"))g.remove();const{comicId:m}=await l();return t.render(()=>{const g=a.useComicDetail(m);return t.createComponent(o.For,{get each(){return g.chapters},children:({name:b,list:w})=>[(()=>{var p=t.template('<div class=photo_part><div class=h2_title2><span class="h2_icon h2_icon22"></span><h2> ')(),s=p.firstChild,y=s.firstChild,_=y.nextSibling,$=_.firstChild;return t.insert(_,()=>g.title,$),t.insert(_,b==="\u8FDE\u8F7D"?"\u5728\u7EBF\u6F2B\u753B\u5168\u96C6":`\u6F2B\u753B\u5176\u5B83\u7248\u672C\uFF1A${b}`,null),p})(),(()=>{var p=t.template("<div class=cartoon_online_border_other><ul></ul><div class=clearfix>")(),s=p.firstChild;return p.style.setProperty("margin-top","-8px"),t.insert(s,t.createComponent(o.For,{each:w,children:({title:y,id:_,updatetime:$})=>(()=>{var I=t.template("<li><a target=_blank>")(),L=I.firstChild;return t.setAttribute(L,"title",y),t.setAttribute(L,"href",`https://m.dmzj.com/view/${m}/${_}.html`),t.insert(L,y),t.effect(()=>L.classList.toggle("color_red",$===g.last_updatetime)),I})()})),p})()]})},n.querySelector(".middleright_mr")),!1},h=async()=>{await n.waitDom("#qiehuan_txt"),await n.wait(()=>{const m=n.querySelector("#qiehuan_txt");if(m){if(m.textContent!=="\u5207\u6362\u5230\u4E0A\u4E0B\u6EDA\u52A8\u9605\u8BFB")return!0;m.click()}})},c=async()=>(await h(),await n.waitDom(".comic_wraCon img"),n.querySelectorAll(".comic_wraCon img").map(m=>m.src)),d=m=>{const g=n.querySelector(m);if(g?.textContent)return()=>g.click()},u=()=>/^\/[^/]*?\/?$/.test(window.location.pathname)?f():/^\/.*?\/\d+\.shtml$/.test(window.location.pathname);await r.universal({name:"dmzj",getImgList:c,onExit:m=>m&&n.scrollIntoView("#hd"),async getCommentList(){const{comicId:m,chapterId:g}=await l();return a.getViewpoint(m,g)},SPA:{isMangaPage:u,getOnPrev:()=>d(".display_left #prev_chapter"),getOnNext:()=>d(".display_right #next_chapter")}})})().catch(l=>n.log.error(l));break}case"m.idmzj.com":case"m.dmzj.com":{const t=require("dmzjDecrypt"),o=require("userscript/dmzjApi"),r=require("main"),a=require("helper");(async()=>{switch(window.location.pathname.split("/")[1]){case"info":{if(Reflect.has(unsafeWindow,"obj_id"))return;const n=Number.parseInt(window.location.pathname.split("/")[2],10);if(Number.isNaN(n)){document.body.innerHTML="",document.body.insertAdjacentHTML("beforeend",`
            \u8BF7\u624B\u52A8\u8F93\u5165\u6F2B\u753B\u540D\u8FDB\u884C\u641C\u7D22 <br />
            <input type="search"> <button>\u641C\u7D22</button> <br />
            <div id="list" />
          `),a.querySelector("button").addEventListener("click",async()=>{const d=a.querySelector("input")?.value;if(!d)return;const u=await r.request(`https://s.acg.dmzj.com/comicsum/search.php?s=${d}`,{errorText:"\u641C\u7D22\u6F2B\u753B\u65F6\u51FA\u9519"}),m=JSON.parse(u.responseText.slice(20,-1));a.querySelector("#list").innerHTML=m.map(({id:g,comic_name:b,comic_author:w,comic_url:p})=>`
                <b>\u300A${b}\u300B<b/>\u2014\u2014${w}
                <a href="${p}">Web\u7AEF</a>
                <a href="https://m.dmzj.com/info/${g}.html">\u79FB\u52A8\u7AEF</a>
              `).join("<br />")});return}const l=await r.request(`https://v4api.idmzj.com/comic/detail/${n}?uid=2665531&disable_level=1`,{errorText:"\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25"}),{comicInfo:{last_updatetime:f,title:h,chapters:c}}=t(l.responseText);document.title=h,document.body.insertAdjacentHTML("beforeend",`<h1>${h}</h1>`);for(const d of Object.values(c)){let u=`<h2>${d.title}</h2>`,m=d.data.length;for(;m--;)u+=`<a target="_blank" title="${d.data[m].chapter_title}" href="https://m.dmzj.com/view/${n}/${d.data[m].chapter_id}.html" ${d.data[m].updatetime===f?'style="color:red"':""}>${d.data[m].chapter_title}</a>`;document.body.insertAdjacentHTML("beforeend",u)}document.body.childNodes[0].remove(),GM_addStyle(`
          h1 {
            margin: 0 -20vw;
          }
        ...
        `);break}case"view":{if(unsafeWindow.comic_id){GM_addStyle(".subHeader{display:none !important}"),await r.universal({name:"dmzj",getImgList:()=>a.querySelectorAll("#commicBox img").map(s=>s.dataset.original).filter(Boolean),getCommentList:()=>o.getViewpoint(unsafeWindow.subId,unsafeWindow.chapterId),onNext:a.querySelectorClick("#loadNextChapter"),onPrev:a.querySelectorClick("#loadPrevChapter")});return}const n=document.createElement("p");n.textContent="\u6B63\u5728\u52A0\u8F7D\u4E2D\uFF0C\u8BF7\u5750\u548C\u653E\u5BBD\uFF0C\u82E5\u957F\u65F6\u95F4\u65E0\u53CD\u5E94\u8BF7\u5237\u65B0\u9875\u9762",document.body.append(n);let l,f,h;try{[,f,h]=/(\d+)\/(\d+)/.exec(window.location.pathname),l=await o.getChapterInfo(f,h)}catch(s){throw r.toast.error("\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25",{duration:Number.POSITIVE_INFINITY}),n.textContent=s.message,s}n.textContent="\u52A0\u8F7D\u5B8C\u6210\uFF0C\u5373\u5C06\u8FDB\u5165\u9605\u8BFB\u6A21\u5F0F";const{folder:c,chapter_name:d,next_chap_id:u,prev_chap_id:m,comic_id:g,page_url:b}=l;document.title=`${d} ${c.split("/").at(1)}`;const{setManga:w,setComicLoad:p}=await r.useInit("dmzj");w({onExit:void 0,onNext:u?()=>{window.location.href=`https://m.dmzj.com/view/${g}/${u}.html`}:void 0,onPrev:m?()=>{window.location.href=`https://m.dmzj.com/view/${g}/${m}.html`}:void 0,editButtonList:s=>s}),p(()=>b.length>0?b:(n.innerHTML='\u65E0\u6CD5\u83B7\u5F97\u6F2B\u753B\u6570\u636E\uFF0C\u8BF7\u901A\u8FC7 <a href="https://github.com/hymbz/ComicReadScript/issues" target="_blank">Github</a> \u6216 <a href="https://sleazyfork.org/zh-CN/scripts/374903-comicread/feedback#post-discussion" target="_blank">Greasy Fork</a> \u8FDB\u884C\u53CD\u9988',[])),w("commentList",await o.getViewpoint(f,h));break}}})().catch(n=>a.log.error(n));break}case"www.idmzj.com":case"www.dmzj.com":{const t=require("userscript/dmzjApi"),o=require("main"),r=require("helper"),a=n=>{if(n)return()=>{window.open(window.location.href.replace(/(?<=\/)\d+(?=\.html)/,`${n}`),"_self")}};(async()=>{await r.waitDom(".head_wz");const n=r.querySelector(".head_wz [id]")?.id,l=/(?<=\/)\d+(?=\.html)/.exec(window.location.pathname)?.[0];if(!n||!l)return;const{setManga:f,setComicLoad:h}=await o.useInit("dmzj");try{const{next_chap_id:c,prev_chap_id:d,page_url:u}=await t.getChapterInfo(n,l);h(()=>u),f({onNext:a(c),onPrev:a(d)})}catch{o.toast.error("\u83B7\u53D6\u6F2B\u753B\u6570\u636E\u5931\u8D25",{duration:Number.POSITIVE_INFINITY})}})().catch(n=>r.log.error(n));break}case"exhentai.org":case"e-hentai.org":{const web=require("solid-js/web"),solidJs=require("solid-js"),main=require("main"),Manga=require("components/Manga"),detectAd=require("userscript/detectAd"),helper=require("helper"),store=require("solid-js/store"),ehTagRules=require("userscript/ehTagRules"),escHandler=[],setEscHandler=(t,o)=>{escHandler.push(Object.assign(o,{order:t})),escHandler.sort((r,a)=>a.order-r.order)},getTaglist=()=>{const t=new Set,o=new Set;for(const r of helper.querySelectorAll("#taglist table [id^=td_]")){const[a]=r.getElementsByTagName("a");a.classList.contains("tdn")||(a.classList.contains("tup")||r.classList.contains("gt")?t.add(r.id.slice(3)):r.classList.contains("gtl")&&o.add(r.id.slice(3)))}return[t,o]},handleTagName=t=>{const[o,r]=t.trim().split(":");return r?[o,r.replaceAll(/[^a-zA-Z-_ ]/g,"")]:["",""]},namespaceAbbr=[["artist","a"],["character","c","char"],["cosplayer","c","os"],["female","f"],["group","g","circle"],["language","l","lang"],["male","m"],["mixed","x"],["other","o"],["parody","p","series"],["reclass","r"]],getTagNameFull=t=>{const[o,r]=handleTagName(t);for(const a of namespaceAbbr)if(a.includes(o))return`${a[0]}:${r}`;return t};let hasStyle=!1;const addQuickFavorite=(favoriteButton,root,apiUrl,position)=>{hasStyle||(hasStyle=!0,GM_addStyle(`
      .comidread-favorites {
        position: absolute;
        z-index: 75;
        left: 0;
    ...
    `)),root.style.position="relative",root.style.height="100%";const[show,setShow]=solidJs.createSignal(!1),[favorites,setFavorites]=solidJs.createSignal([]),[favnote,setFavnote]=solidJs.createSignal(""),updateFavorite=async()=>{try{const t=await main.request(apiUrl,{errorText:helper.t("site.ehentai.fetch_favorite_failed")}),o=helper.domParse(t.responseText),r=[...o.querySelectorAll(".nosel > div")];r.length===10&&(r[0].querySelector("input").checked=!1),setFavnote(o.querySelector('#galpop textarea[name="favnote"]')?.value??""),setFavorites(r)}catch{main.toast.error(helper.t("site.ehentai.fetch_favorite_failed")),setFavorites([])}};let hasRender=!1;const renderDom=()=>{if(hasRender)return;hasRender=!0;const FavoriteItem=(e,index)=>{const checked=e.querySelector("input").checked,handleClick=async()=>{if(checked)return;setShow(!1);const formData=new FormData;formData.append("favcat",index()===10?"favdel":`${index()}`),formData.append("apply","Apply Changes"),formData.append("favnote",favnote()),formData.append("update","1");const res=await main.request(apiUrl,{method:"POST",data:formData,errorText:helper.t("site.ehentai.change_favorite_failed")});main.toast.success(helper.t("site.ehentai.change_favorite_success"));const updateCode=/\nif\(window.opener.document.+\n/.exec(res.responseText)?.[0]?.replaceAll("window.opener.document","window.document");updateCode&&eval(updateCode),await updateFavorite()};return(()=>{var t=web.template("<div class=comidread-favorites-item><input type=radio>")(),o=t.firstChild;return t.$$click=handleClick,o.checked=checked,web.insert(t,web.createComponent(solidJs.Show,{get when(){return index()<=9},get children(){var r=web.template("<div>")();return web.effect(a=>(a=`0px -${2+19*index()}px`)!=null?r.style.setProperty("background-position",a):r.style.removeProperty("background-position")),r}}),null),web.insert(t,()=>e.textContent?.trim(),null),t})()};let background="rgba(0, 0, 0, 0)",dom=root;for(;background==="rgba(0, 0, 0, 0)";)background=getComputedStyle(dom).backgroundColor,dom=dom.parentElement;web.render(()=>web.createComponent(solidJs.Show,{get when(){return show()},get children(){var t=web.template("<span class=comidread-favorites>")();return background!=null?t.style.setProperty("background",background):t.style.removeProperty("background"),web.insert(t,web.createComponent(solidJs.For,{get each(){return favorites()},children:FavoriteItem,get fallback(){return web.template("<h3>loading...")()}})),web.effect(o=>{var r=`${position[1]-position[0]}px`,a=`${position[0]}px`;return r!==o.e&&((o.e=r)!=null?t.style.setProperty("height",r):t.style.removeProperty("height")),a!==o.t&&((o.t=a)!=null?t.style.setProperty("top",a):t.style.removeProperty("top")),o},{e:void 0,t:void 0}),t}}),root)},rawClick=favoriteButton.onclick;favoriteButton.onclick=null,favoriteButton.addEventListener("mousedown",async t=>{if(!(t.buttons!==1&&t.buttons!==4)){if(t.stopPropagation(),t.preventDefault(),t.shiftKey||t.ctrlKey||t.altKey||t.metaKey||t.buttons===4)return rawClick.call(favoriteButton,t);renderDom(),setShow(o=>!o),show()&&await updateFavorite()}})},quickFavorite=t=>{if(t==="gallery"){const o=helper.querySelector("#gdf"),r=helper.querySelector("#gd3");addQuickFavorite(o,r,`${unsafeWindow.popbase}addfav`,[0,o.firstElementChild.offsetTop]);return}switch(t){case"t":{for(const o of helper.querySelectorAll(".gl1t")){const r=o.querySelector("[id^=posted_]"),a=o.firstElementChild.getBoundingClientRect().bottom-o.getBoundingClientRect().top,n=o.lastElementChild.getBoundingClientRect().top-o.getBoundingClientRect().top,l=/http.+?(?=')/.exec(r.getAttribute("onclick"))[0];addQuickFavorite(r,o,l,[a,n])}break}case"e":{for(const o of helper.querySelectorAll(".gl1e")){const r=o.nextElementSibling.querySelector("[id^=posted_]"),a=Number.parseInt(getComputedStyle(o).height,10),n=/http.+?(?=')/.exec(r.getAttribute("onclick"))[0];addQuickFavorite(r,o,n,[0,a])}break}}};web.delegateEvents(["click"]);const associateNhentai=async(t,o,r)=>{if(!helper.querySelector("#gdc > .cs:is(.ct2, .ct3)"))return;const a=document.getElementById("gn");if(!a||!helper.querySelector("#taglist tbody")){(document.getElementById("taglist")?.children.length??1)>0&&main.toast.error(helper.t("site.ehentai.html_changed_nhentai_failed"));return}const[n,l]=solidJs.createSignal(),f=a.textContent.replaceAll(/\s+-/g," "),h=()=>{if(n()===void 0)return"searching...";if(n()===null){const w=`https://nhentai.net/search/?q=${f}`;return helper.t("site.ehentai.nhentai_failed",{nhentai:`<a href='${w}' target="_blank"> <u> nhentai </u> </a>`})}if(n().length===0)return"null"},c=()=>(()=>{var w=web.template("<tr id=nh_tagline><td class=tc>nhentai:")();return w.firstChild,web.insert(w,web.createComponent(solidJs.Show,{get when(){return n()?.length},get fallback(){return(()=>{var p=web.template("<td class=tc>")();return p.style.setProperty("text-align","left"),web.effect(()=>p.innerHTML=h()),p})()},get children(){var p=web.template("<td>")();return web.insert(p,web.createComponent(solidJs.For,{get each(){return n()},children:({id:s,title:y})=>(()=>{var _=web.template("<div class=gtl><a>")(),$=_.firstChild;return web.setAttribute(_,"id",`td_nh:${s}`),_.style.setProperty("opacity","1.0"),web.setAttribute($,"id",`nh:${s}`),web.setAttribute($,"href",`https://nhentai.net/g/${s}/`),web.setAttribute($,"onclick",`return toggle_tagmenu(1, 'nh:${s}',this)`),web.insert($,s),web.effect(()=>web.setAttribute(_,"title",y.japanese||y.english)),_})()})),p}}),null),w})();web.render(c,helper.querySelector("#taglist tbody")),helper.hijackFn("tag_update_vote",()=>{for(const w of helper.querySelectorAll("#nh_tagline"))w.remove();web.render(c,helper.querySelector("#taglist tbody"))});try{const w=await main.request(`https://nhentai.net/api/galleries/search?query=${f}`,{responseType:"json",errorText:helper.t("site.ehentai.nhentai_error"),noTip:!0,headers:{"User-Agent":navigator.userAgent}});l(w.response.result)}catch{l(null)}if(!n()?.length)return;const d={j:"jpg",p:"png",g:"gif",w:"webp",b:"bmp"};for(const{id:w,images:p,num_pages:s,media_id:y}of n()){const _=`nh:${w}`;o(t(I=>{helper.plimit(p.pages.map((L,M)=>async()=>{const x=await main.request(`https://i.nhentai.net/galleries/${y}/${M+1}.${d[L.t]}`,{headers:{Referer:`https://nhentai.net/g/${y}`},responseType:"blob"}),A=URL.createObjectURL(x.response);I(M,A)}))},s,_),_)}const u=document.getElementById("tagmenu_act"),m=()=>web.template('<img src=https://ehgt.org/g/mr.gif class=mr alt=">">')(),g=w=>web.createComponent(solidJs.For,{get each(){return w.children},children:p=>[m(),p]});let b;helper.hijackFn("_refresh_tagmenu_act",(w,[p])=>{if(b?.(),!p.id.startsWith("nh:"))return w(p);u.children.length>0&&(u.innerHTML=""),b=web.render(()=>web.createComponent(g,{get children(){return[(()=>{var s=web.template("<a target=_blank>")();return s.innerText=" Jump to nhentai",web.effect(()=>web.setAttribute(s,"href",p.href)),s})(),web.createComponent(r,{get id(){return p.id}})]}}),u)})},hotkeysPageTurn=t=>{t==="gallery"?(setEscHandler(0,()=>unsafeWindow.selected_tagname?unsafeWindow.toggle_tagmenu():!0),helper.linstenKeydown(o=>{switch(o.key){case"ArrowRight":case"d":return o.preventDefault(),helper.querySelector(".ptt td:last-child:not(.ptdd)")?.click();case"ArrowLeft":case"a":return o.preventDefault(),helper.querySelector(".ptt td:first-child:not(.ptdd)")?.click()}if(unsafeWindow.selected_tagid)switch(o.key){case"ArrowUp":return o.preventDefault(),unsafeWindow?.tag_vote_up();case"ArrowDown":return o.preventDefault(),unsafeWindow?.tag_vote_down()}})):helper.linstenKeydown(o=>{switch(o.key){case"ArrowRight":case"d":return o.preventDefault(),helper.querySelector("#unext")?.click();case"ArrowLeft":case"a":return o.preventDefault(),helper.querySelector("#uprev")?.click()}})},getTagSetHtml=async t=>{const o=t?`/mytags?tagset=${t}`:"/mytags",r=await main.request(o,{fetch:!0});return helper.domParse(r.responseText)},collectTags=(t,o=[])=>{const r=t.querySelector("#tagcolor").value.slice(1)||"0",[,...a]=[...t.getElementById("usertags_outer").children];for(const n of a){const l=Number(n.id.split("usertag_")[1]),f=n.querySelector(`#tagpreview_${l}`),{color:h,borderColor:c}=f.style;let[d,u]=f.title.split(":");switch(d){case"female":case"male":case"mixed":d="gender"}const m=Number.parseInt(n.querySelector(`#tagcolor_${l}`).value.slice(1)||r,16);o.push({e:n,id:l,title:f.title,color:m,fontColor:h,borderColor:c,group:d,name:u,weight:Number(n.querySelector("input[id^=tagweight_]").value),watch:n.querySelector(`#tagwatch_${l}`).checked,hidden:n.querySelector(`#taghide_${l}`).checked,order:-1})}return o},sortTagList=t=>{const o=new Intl.Collator,r=(n,l)=>n.color!==l.color?l.color-n.color:n.group!==l.group?o.compare(n.group,l.group):n.hidden!==l.hidden?n.hidden?1:-1:n.watch!==l.watch?n.watch?-1:1:n.weight!==l.weight?l.weight-n.weight:o.compare(n.name,l.name);let a=-t.length;for(const n of t.sort(r))n.order=a++;return t},getMyTags=async()=>{const t=[],o=await getTagSetHtml();await Promise.all([...o.querySelectorAll("#tagset_outer select option")].map(async a=>{const n=a.selected?o:await getTagSetHtml(a.value);n.querySelector("#tagset_enable")?.checked&&t.push(n)}));const r=[];for(const a of t)collectTags(a,r);return sortTagList(r)},handleMyTagsChange=new Set,updateMyTags=async()=>{const t=await getMyTags();for(const o of handleMyTagsChange)await o(t)},buildTagList=(t,o)=>`
${[...t].map(r=>`${o}${CSS.escape(r)}`).join(`,
`)}
`,updateTagColor=async t=>{const o={},r={},a={};for(const l of t){const{color:f,borderColor:h,fontColor:c}=l,d=l.title.replaceAll(" ","_");(o[f]||=new Set).add(d),(r[h]||=new Set).add(d),(a[c]||=new Set).add(d)}let n="";for(const[l,f]of Object.entries(o))n+=`:is(${buildTagList(f,"#td_")})`,n+=`{ background: #${Number(l).toString(16).padStart(6,"0")}; }

`;for(const[l,f]of Object.entries(r))n+=`:is(${buildTagList(f,"#td_")}).gt`,n+=`{ border-color: ${l}; }

`;for(const[l,f]of Object.entries(a))n+=`:is(${buildTagList(f,"#td_")}):not(.gt)`,n+=`{ border-color: ${l}; }

`,n+=`#taglist a:is(${buildTagList(f,"#ta_")})`,n+=`{ color: ${l} !important; position: relative; }
...
`;return n+=`
  ...
  `,await GM.setValue("ehTagColorizeCss",n),n},colorizeTag=async t=>{switch(handleMyTagsChange.add(updateTagColor),t){case"gallery":{let o=location.origin==="https://exhentai.org"?"--tag: #DDDDDD; --tag-hover: #EEEEEE; --tup: #00E639; --tdn: #FF3333;":"--tag: #5C0D11; --tag-hover: #8F4701; --tup: green; --tdn: red;";return o=`#taglist { ${o} }

`,o+=await helper.getGmValue("ehTagColorizeCss",updateMyTags),GM_addStyle(o)}case"mytags":updateMyTags(),helper.hijackFn("usertag_callback",helper.debounce(updateMyTags))}},quickRating=t=>{let o;switch(t){case"gallery":case"mytags":case"mpv":return;case"e":o=helper.querySelectorAll("#favform > table > tbody > tr");break;case"m":case"p":case"l":o=helper.querySelectorAll("#favform > table > tbody > tr").slice(1);break;case"t":o=helper.querySelectorAll(".gl1t");break}GM_addStyle(`
  ...
  `);const r=["0,0,7,16","8,0,15,16","16,0,23,16","24,0,31,16","32,0,39,16","40,0,47,16","48,0,55,16","56,0,63,16","64,0,71,16","72,0,79,16"],a=async(f,h)=>{try{const c=await main.request(f,{errorText:helper.t("site.ehentai.change_rating_failed"),noTip:!0}),d=/api_url = "(.+?)".+?gid = (\d+).+?token = "(.+?)".+?apiuid = (\d+).+?apikey = "(.+?)"/s.exec(c.responseText);if(!d)throw new Error(helper.t("site.ehentai.change_rating_failed"));const[,u,m,g,b,w]=d,p=await main.request(u,{method:"POST",responseType:"json",data:JSON.stringify({method:"rategallery",rating:`${h}`,apikey:w,apiuid:b,gid:m,token:g}),fetch:!0,noTip:!0});return main.toast.success(`${helper.t("site.ehentai.change_rating_success")}: ${p.response.rating_usr}`),p.response}catch{throw main.toast.error(helper.t("site.ehentai.change_rating_failed")),new Error(helper.t("site.ehentai.change_rating_failed"))}},n=(f,h)=>{let c=Math.round(h+1);const d=-80+16*Math.ceil(c/2);c=c%2===1?-21:-1,f.style.backgroundPosition=`${d}px ${c}px`},l=(f,h,c)=>{let d=h.style.backgroundPosition;web.render(()=>(()=>{var u=web.template("<span class=comidread-quick-rating><img src=https://ehgt.org/g/blank.gif><map>")(),m=u.firstChild,g=m.nextSibling;return u.$$mouseout=()=>{h.style.backgroundPosition=d},web.setAttribute(u,"data-index",c),web.setAttribute(m,"usemap",`#rating-${c}`),web.setAttribute(g,"name",`rating-${c}`),web.insert(g,web.createComponent(solidJs.For,{each:r,children:(b,w)=>(()=>{var p=web.template("<area shape=rect>")();return p.$$click=async()=>{const s=await a(f.querySelector("a").href,w()+1);h.className=s.rating_cls,n(h,s.rating_usr*2-1),d=h.style.backgroundPosition},p.$$mouseover=()=>n(h,w()),web.setAttribute(p,"coords",b),p})()})),u})(),h)};for(const[f,h]of o.entries()){const c=[...h.querySelectorAll(".ir")].at(-1);c&&c.addEventListener("mouseenter",()=>l(h,c,f),{once:!0})}};web.delegateEvents(["mouseout","mouseover","click"]);const MDLaunch=(t={})=>(()=>{var o=web.template('<svg xmlns=http://www.w3.org/2000/svg viewBox="0 0 24 24"stroke=currentColor fill=currentColor stroke-width=0><path d="M18 19H6c-.55 0-1-.45-1-1V6c0-.55.45-1 1-1h5c.55 0 1-.45 1-1s-.45-1-1-1H5a2 2 0 0 0-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-6c0-.55-.45-1-1-1s-1 .45-1 1v5c0 .55-.45 1-1 1M14 4c0 .55.45 1 1 1h2.59l-9.13 9.13a.996.996 0 1 0 1.41 1.41L19 6.41V9c0 .55.45 1 1 1s1-.45 1-1V3h-6c-.55 0-1 .45-1 1">')();return web.spread(o,t,!0,!0),o})(),quickTagDefine=t=>{if(t!=="gallery")return;const o=store.createMutable({}),r=async c=>{if(Reflect.has(o,c))return;const d=`https://ehwiki.org/wiki/${c.replaceAll(/[a-z]+:\s?/gi,"")}`,u=await main.request(d,{noCheckCode:!0});if(u.status!==200){o[c]=(()=>{var b=web.template("<h3>")();return web.insert(b,()=>`${u.status} - ${u.statusText}`),b})();return}const g=helper.domParse(u.responseText).querySelector("#mw-content-text");for(const b of g.querySelectorAll('img[src^="/"]'))b.setAttribute("src",`https://ehwiki.org${b.getAttribute("src")}`);for(const b of g.getElementsByTagName("a")){const w=b.getAttribute("href")??"";w.startsWith("/")&&b.setAttribute("href",`https://ehwiki.org${w}`),b.target="_blank"}for(const b of g.querySelectorAll(".thumb"))b.remove();o[c]=[(()=>{var b=web.template("<h1><a target=_blank>")(),w=b.firstChild;return web.setAttribute(w,"href",d),web.insert(w,c,null),web.insert(w,web.createComponent(MDLaunch,{}),null),b})(),g]};GM_addStyle(`
  ...
  `);const[a,n]=solidJs.createSignal(!1),l=helper.querySelector("#taglist");let f="rgba(0, 0, 0, 0)",h=l;for(;f==="rgba(0, 0, 0, 0)";)f=getComputedStyle(h).backgroundColor,h=h.parentElement;web.render(()=>web.createComponent(solidJs.Show,{get when(){return a()},get children(){var c=web.template("<span id=comidread-tag-define>")();return f!=null?c.style.setProperty("background",f):c.style.removeProperty("background"),web.insert(c,()=>o[unsafeWindow.selected_tagname]??web.template("<h3>loading...")()),web.effect(d=>(d=`${l.scrollHeight}px`)!=null?c.style.setProperty("height",d):c.style.removeProperty("height")),c}}),l),unsafeWindow.tag_define=async()=>{if(unsafeWindow.selected_tagname){if(a())return n(!1);n(!0);try{await r(unsafeWindow.selected_tagname)}catch(c){console.error(c),n(!1)}}},helper.hijackFn("toggle_tagmenu",()=>n(!1)),setEscHandler(2,()=>a()?n(!1):!0)},MdPictureInPicture='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" fill="currentColor" stroke-width="0"><path d="M18 7h-6c-.55 0-1 .45-1 1v4c0 .55.45 1 1 1h6c.55 0 1-.45 1-1V8c0-.55-.45-1-1-1m3-4H3c-1.1 0-2 .9-2 2v14c0 1.1.9 1.98 2 1.98h18c1.1 0 2-.88 2-1.98V5c0-1.1-.9-2-2-2m-1 16.01H4c-.55 0-1-.45-1-1V5.98c0-.55.45-1 1-1h16c.55 0 1 .45 1 1v12.03c0 .55-.45 1-1 1"/></svg>',getDomPosition=t=>{const o=t.getBoundingClientRect(),r=getComputedStyle(t),a=Number.parseFloat(r.borderLeftWidth),n=Number.parseFloat(r.paddingLeft),l=Number.parseFloat(r.paddingTop),f=Number.parseFloat(r.borderTopWidth);return{left:o.left+a+n,top:o.top+f+l,width:r.width,height:r.height}},floatTagList=(t,o)=>{if(t!=="gallery")return;const r=helper.querySelector("#gd4"),a=getComputedStyle(r);let n="rgba(0, 0, 0, 0)",l=r;for(;n==="rgba(0, 0, 0, 0)";)n=getComputedStyle(l).backgroundColor,l=l.parentElement;const{borderColor:f}=getComputedStyle(helper.querySelector("#gdt")),h=`1px solid ${f}`;GM_addStyle(`
      #comicread-tag-box {
        position: fixed;
        z-index: 2147483647;
    ...
    `);const{store:c,setState:d,_setState:u,_state:m}=helper.useStore({open:!1,top:0,left:0,opacity:1,mouse:{x:0,y:0},bound:{width:0,height:0}}),g=(v,S,T)=>{v.top=helper.clamp(-r.clientHeight*.75,S,v.bound.height),v.left=helper.clamp(-r.clientWidth*.75,T,v.bound.width)},b=v=>u("opacity",helper.clamp(.5,v,1));b(Number(localStorage.getItem("floatTagListOpacity"))||1),document.addEventListener("pointermove",v=>{d(S=>{S.mouse.x=v.clientX,S.mouse.y=v.clientY})});const w=()=>{d(v=>{v.bound.width=window.innerWidth-r.clientWidth/4,v.bound.height=window.innerHeight-r.clientHeight/4,g(v,v.top,v.left)})};window.addEventListener("resize",w),w(),helper.useStyleMemo("#comicread-tag-box",{display:()=>c.open?void 0:"none",top:()=>`${c.top}px`,left:()=>`${c.left}px`,opacity:()=>c.opacity});const p=r.cloneNode();p.id="comicread-tag-box-placeholder",p.style.display="none",p.addEventListener("click",()=>u("open",!1)),p.innerHTML=MdPictureInPicture,r.before(p);const s=document.createElement("div");s.id="comicread-tag-box",s.classList.add("comicread-ignore"),document.body.append(s),s.addEventListener("wheel",v=>{v.shiftKey&&(v.stopPropagation(),v.preventDefault(),b(c.opacity+(v.deltaY>0?-.05:.05)),localStorage.setItem("floatTagListOpacity",`${c.opacity}`))},{passive:!1});const y={top:0,left:0};helper.useDrag({ref:r,handleDrag({type:v,xy:[S,T],initial:[R,E]}){switch(v){case"down":if(!c.open){const C=getDomPosition(r);d(P=>{P.top=C.top,P.left=C.left})}y.top=c.top,y.left=c.left;break;case"up":d(C=>{if(o.show)return;const P=p.getBoundingClientRect();helper.approx(C.top,P.top,50)&&helper.approx(C.left,P.left,50)&&(C.open=!1)});break;case"move":d(C=>{g(C,y.top+T-E,y.left+S-R),C.open=!0});break}},handleClick:(v,S)=>S.click(),skip:v=>!v.target.matches("#gd4, #taglist, #gwrd, td+td, [id^=comidread] *:not(a)")});let _,$;const I=()=>{if(_||(_=helper.querySelector("#ehs-introduce-box"),!_))return;$=_.parentElement;const v=helper.querySelector(".eh-syringe-lite-auto-complete-list");v&&(v.classList.add("comicread-ignore"),v.style.zIndex="2147483647",document.body.append(v)),helper.hijackFn("toggle_tagmenu",()=>unsafeWindow.selected_tagname||helper.querySelector("#ehs-introduce-box .ehs-close")?.click())};helper.createEffectOn(()=>c.open,v=>{if(I(),v){const{height:S,width:T}=a;p.style.cssText=`height: ${S}; width: ${T};`,s.style.height=S,r.style.width=T,s.append(r),_&&s.append(_),document.activeElement.blur()}else p.style.cssText="display: none;",r.style.width="",p.after(r),_&&$.append(_),Manga.focus()},{defer:!0}),Manga.setDefaultHotkeys(v=>({...v,float_tag_list:["q"]})),setEscHandler(0,()=>c.open?u("open",!1):!0),helper.linstenKeydown(v=>{const S=helper.getKeyboardCode(v);Manga.hotkeysMap()[S]==="float_tag_list"&&(v.stopPropagation(),v.preventDefault(),d(T=>{T.open=!T.open,T.open&&g(T,T.mouse.y-r.clientHeight/2,T.mouse.x-r.clientWidth/2)}))}),helper.hijackFn("tag_from_field",(v,S)=>(c.open&&document.activeElement.blur(),v(...S)));const L=helper.querySelector("#newtagfield");L.addEventListener("pointerenter",()=>c.open&&L.focus());const M=v=>{const S=helper.querySelector(`a[href=${CSS.escape(v)}]`);if(S)return S.title||S.id.slice(3).replaceAll("_"," ")},x=v=>{const S=v.dataTransfer.getData("text"),T=M(S);T&&(v.preventDefault(),L.value.includes(T)||(L.value+=`${T}, `),L.dispatchEvent(new Event("input")))};L.addEventListener("drop",x);const A=helper.querySelector("#taglist");A.addEventListener("dragover",v=>v.preventDefault()),A.addEventListener("dragenter",v=>v.preventDefault()),A.addEventListener("drop",x)},updateSortCss=t=>{let o="tr a :is(.gltm, .glink + div:not([class])) { display: flex; }";for(const{title:r,order:a}of t)o+=`
.gt[title="${r}"] { order: ${a}; }`;return GM.setValue("ehTagSortCss",o)},sortTags=async t=>{switch(handleMyTagsChange.add(updateSortCss),t){case"p":case"l":case"t":return GM_addStyle(await helper.getGmValue("ehTagSortCss",updateMyTags));case"mytags":{let o;const r=a=>{let n=`
          #usertags_outer { display: flex; flex-direction: column; }
          #usertags_outer > div { margin: unset; }
          #usertag_0 { order: -${a.length}; }`;for(const{order:l,id:f}of a)n+=`
#usertag_${f} { view-transition-name: _${f}; order: ${l}; }`;o||=GM_addElement("style",{textContent:n}),o.textContent=n};handleMyTagsChange.add(a=>{if(!document.startViewTransition)return r(a);document.startViewTransition(()=>r(a))})}}},tagLint=t=>{if(t!=="gallery")return;const o=helper.querySelector("#gdc > .cs:is(.ct2, .ct3, .ct9)"),r=ehTagRules.getTagLintRules(),[a,n]=solidJs.createSignal({});GM_addStyle(`
    #comidread-tag-lint [id^=td_] {
      display: inline-block;
      float: none;
    }
  `);const l=(s,y)=>y===void 0?document.getElementById(`td_${s}`)?.className:y?"gtl":"gt",f=s=>(()=>{var y=web.template("<div><a>")(),_=y.firstChild;return _.$$click=$=>$.preventDefault(),web.insert(_,()=>s.name),web.effect($=>{var I=`td_${s.name}`,L=l(s.name,s.weak),M=`ta_${s.name}`,x=`https://exhentai.org/tag/${s.name.replaceAll("_","+")}`;return I!==$.e&&web.setAttribute(y,"id",$.e=I),L!==$.t&&web.className(y,$.t=L),M!==$.a&&web.setAttribute(_,"id",$.a=M),x!==$.o&&web.setAttribute(_,"href",$.o=x),$},{e:void 0,t:void 0,a:void 0,o:void 0}),y})(),h=s=>{const y=ehTagRules.splitTagNamespace(s.name);return web.createComponent(solidJs.Show,{get when(){return y.length>1},get fallback(){return f(s)},get children(){var _=web.template("<span>\u300C<!> \u300D")(),$=_.firstChild,I=$.nextSibling;return I.nextSibling,web.insert(_,web.createComponent(solidJs.For,{each:y,children:(L,M)=>[web.memo(()=>web.memo(()=>!!M())()?` ${helper.t("other.or")} `:""),web.createComponent(f,{name:L,get weak(){return s.weak}})]}),I),_}})},c=s=>{const[y,_,$]=s.text.split("[tag]");return web.createComponent(solidJs.Show,{get when(){return s.warnList?.size},get children(){return web.createComponent(solidJs.For,{get each(){return[...s.warnList.entries()]},children:([I,L])=>(()=>{var M=web.template("<li>")();return web.insert(M,y,null),web.insert(M,web.createComponent(h,{name:I}),null),web.insert(M,_,null),web.insert(M,web.createComponent(solidJs.For,{each:L,children:x=>web.createComponent(h,{name:x,get weak(){return s.weak}})}),null),web.insert(M,$,null),M})()})}})};let d,u;const m=helper.singleThreaded(()=>{const s={},[y,_]=getTaglist(),$=new Set([...y,..._]),I=(x,A,v=!1)=>{const S=r[A];if(S.has(x))for(const T of S.get(x)){if(ehTagRules.hasTag(v?y:$,T)===v)continue;s[A]||=new Map([[x,[]]]);const R=s[A];R.has(x)||R.set(x,[]),R.get(x).push(T)}};for(const x of $)I(x,"prerequisite",!0),I(x,"conflict"),o&&I(x,"possibleConflict"),I(x,"combo",!0);const L=(x,A)=>{s.other||=[],s.other.push([x,A])},M=[];for(const x of _)if(/^(artist|group):/.test(x)){const A=helper.querySelector("#gd2").textContent.toLowerCase();if(A.includes(x.replaceAll(/^(artist|group):|_/g," ").trim()))M.push(x);else{const v=document.getElementById(`ta_${x}`)?.textContent;v&&A.includes(v)&&M.push(x)}}M.length>0&&L(helper.t("eh_tag_lint.correct_tag"),M),helper.querySelector("#gdc > .cs.ct2")&&ehTagRules.isMissingNamespace($,"parody")&&L(helper.t("eh_tag_lint.miss_parody"),["parody:original"]),o&&ehTagRules.isMissingTags(y,"female:females_only","female:futanari","female:shemale")&&ehTagRules.isMissingNamespace($,"male","mixed")&&L(helper.t("eh_tag_lint.miss_female"),["female:females_only"]),n(s),d?.isConnected||(d=document.createElement("div"),d.id="comidread-tag-lint",helper.querySelector("#taglist").append(d)),u?.(),u=web.render(()=>web.createComponent(solidJs.Show,{get when(){return Object.keys(a()).length},get children(){return[web.template("<hr>")(),(()=>{var x=web.template("<ul>")();return web.insert(x,web.createComponent(solidJs.For,{get each(){return a().other},children:([A,v])=>(()=>{var S=web.template("<li>")();return web.insert(S,A,null),web.insert(S,web.createComponent(solidJs.For,{each:v,children:T=>web.createComponent(h,{name:T,weak:!0})}),null),S})()}),null),web.insert(x,web.createComponent(c,{get warnList(){return a().prerequisite},get text(){return helper.t("eh_tag_lint.prerequisite")},weak:!1}),null),web.insert(x,web.createComponent(c,{get warnList(){return a().conflict},get text(){return helper.t("eh_tag_lint.conflict")}}),null),web.insert(x,web.createComponent(c,{get warnList(){return a().possibleConflict},get text(){return helper.t("eh_tag_lint.possible_conflict")}}),null),web.insert(x,web.createComponent(c,{get warnList(){return a().combo},get text(){return helper.t("eh_tag_lint.combo")},weak:!0}),null),x})()]}}),d)});m(),helper.hijackFn("tag_update_vote",m);const g=helper.querySelector("#newtagfield"),[b,w]=helper.createEqualsSignal([]);helper.useStyle(helper.createRootMemo(()=>b().map(s=>`#td_${CSS.escape(s.replaceAll(" ","_"))} { box-shadow: 0px 0px 4px var(--tag); }`).join(`
`)));const p=()=>w(g.value.split(",").map(s=>getTagNameFull(s.trim())).filter(Boolean));g.addEventListener("input",p),g.addEventListener("keydown",p),helper.hijackFn("tag_update_vote",p)};web.delegateEvents(["click"]),(async()=>{let t;if(Reflect.has(unsafeWindow,"display_comment_field")?t="gallery":location.pathname==="/mytags"?t="mytags":Reflect.has(unsafeWindow,"mpvkey")?t="mpv":t=helper.querySelector('option[value="t"]')?.parentElement?.value,!t)return;const{options:o,setComicLoad:r,dynamicLoad:a,showComic:n,comicMap:l,setComicMap:f,setImgList:h,setFab:c,setManga:d,mangaProps:u}=await main.useInit("ehentai",{associate_nhentai:!0,hotkeys:!0,detect_ad:!0,quick_favorite:!0,colorize_tag:!1,quick_rating:!0,quick_tag_define:!0,float_tag_list:!1,auto_adjust_option:!1,tag_lint:!1,autoShow:!1});if(t==="mpv")return r(()=>{const C=helper.querySelectorAll(".mimg[id]");return a(async k=>{const N=unsafeWindow.imagelist;helper.plimit(N.map((O,D)=>async()=>{const q=()=>N[D].i;for(;!q();)Reflect.has(N[D],"xhr")||(unsafeWindow.load_image(D+1),unsafeWindow.next_possible_request=0),await helper.wait(q);k(D,q())}),void 0,4)},C.length)()});if(helper.linstenKeydown(C=>{if(C.key==="Escape"){for(const P of escHandler)if(P()!==!0)return C.stopImmediatePropagation()}}),o.colorize_tag&&(colorizeTag(t),sortTags(t)),o.float_tag_list&&helper.requestIdleCallback(()=>floatTagList(t,u)),unsafeWindow.apiuid!==-1&&o.quick_favorite&&helper.requestIdleCallback(()=>quickFavorite(t)),o.quick_rating&&helper.requestIdleCallback(()=>quickRating(t),1e3),o.quick_tag_define&&helper.requestIdleCallback(()=>quickTagDefine(t),1e3),o.tag_lint&&helper.requestIdleCallback(()=>tagLint(t),1e3),o.auto_adjust_option&&!helper.querySelector("#gdc > .cs:is(.ct2, .ct3, .ct9)")){let C={pageNum:1,imgRecognition:{enabled:!1}};o.option&&(C=helper.assign(o.option,C)),d({option:C})}if(t!=="gallery")return o.hotkeys&&hotkeysPageTurn(t);const m=document.getElementById("gd5");new ResizeObserver(()=>{Reflect.deleteProperty(m.dataset,"long"),m.scrollHeight>352&&(m.dataset.long="")}).observe(m),GM_addStyle(`
    #gd5[data-long] {
  ...
  `);const b=C=>{const P=solidJs.createMemo(()=>{const k=l[C.id]?.imgList,N=k?.filter(Boolean).length;switch(k?.length){case void 0:return" Load comic";case N:return" Read";default:return` loading - ${N}/${k.length}`}});return(()=>{var k=web.template("<a href=javascript:;>")();return k.$$click=async N=>{await C.onClick?.(N),n(C.id)},web.insert(k,P),k})()};o.associate_nhentai&&helper.requestIdleCallback(()=>associateNhentai(a,r,b),1e3);let w=0;w=Number(helper.querySelector(".gtb .gpc")?.textContent?.replaceAll(",","").match(/\d+/g)?.at(-1)),Number.isNaN(w)&&(w=Number(/(?<=<td class="gdt2">)\d+(?= pages<\/td>)/.exec((await main.request(window.location.href)).responseText)?.[0])),Number.isNaN(w)&&main.toast.error(helper.t("site.changed_load_failed"));const p=[],s=[],y=[],_=o.detect_ad&&document.getElementById("ta_other:extraneous_ads");if(_){f("",{adList:new main.ReactiveSet});const C=[];for(const P of helper.querySelectorAll("#gdt > a")){const k=Number(/.+-(\d+)/.exec(P.href)?.[1])-1;if(Number.isNaN(k))continue;s[k]=P.href;const N=P.querySelector("[title]");y[k]=N.title.split(/：|: /)[1],C[k]=N.tagName==="IMG"?N:/url\("(.+)"\)/.exec(N.style.backgroundImage)[1]}(async()=>(await detectAd.getAdPageByFileName(y,l[""].adList),l[""].adList.size===0&&await detectAd.getAdPageByContent(C,l[""].adList),helper.useStyle(helper.createRootMemo(()=>l[""]?.adList?.size?[...l[""].adList].map(P=>`a[href="${s[P]}"] [title]:not(:hover) {
              filter: blur(8px);
              clip-path: border-box;
              backdrop-filter: blur(8px);
            }`).join(`
`):""))))()}const $=C=>C.includes("IP address has been temporarily banned")&&main.toast.error(helper.t("site.ehentai.ip_banned"),{throw:!0,duration:Number.POSITIVE_INFINITY}),I=async C=>{const P=await main.request(C,{fetch:!0,errorText:helper.t("site.ehentai.fetch_img_page_source_failed")},10);$(P.responseText);try{return/id="img" src="(.+?)"/.exec(P.responseText)[1]}catch{throw new Error(helper.t("site.ehentai.fetch_img_url_failed"))}},L=async(C=0)=>{const P=await main.request(`${window.location.pathname}${C?`?p=${C}`:""}`,{fetch:!0,errorText:helper.t("site.ehentai.fetch_img_page_url_failed")});$(P.responseText);const k=[...P.responseText.matchAll(/<a href="(.{20,50})"><(img alt=.+?|div><div |div )title=".+?: (.+?)"/gm)].map(([,N,,O])=>[N,O]);if(k.length===0)throw new Error(helper.t("site.ehentai.fetch_img_page_url_failed"));return k},[M,x]=solidJs.createSignal(`1-${w}`),A=helper.createRootMemo(()=>helper.extractRange(M(),p.length||w)),v=Number(helper.querySelector(".ptt td:nth-last-child(2)").textContent);r(a(async C=>{if(s.length!==v){const P=await helper.plimit(helper.createSequence(v).map(k=>()=>L(k)));s.length=0,y.length=0;for(const k of P)for(const[N,O]of k)s.push(N),y.push(O)}await helper.plimit([...A()].map((P,k)=>async()=>{P<0||(p[P]||=await I(s[P]),C(k,p[P]))})),_&&(detectAd.getAdPageByFileName(y,l[""].adList),detectAd.getAdPageByContent(p,l[""].adList))},()=>A().size));const T=await helper.useCache({pageRange:"id"});web.render(()=>{const C=m.children[6]?.classList.contains("gsp"),P=async k=>{if(!k.shiftKey)return;k.stopPropagation();const N=prompt(helper.t("other.page_range"),(await T.get("pageRange",unsafeWindow.gid))?.range);N&&(await T.set("pageRange",{id:unsafeWindow.gid??Number(location.pathname.split("/")[2]),range:N}),x(N??`1-${w}`),f("","imgList",void 0),n())};return(()=>{var k=web.template('<p class="g2 gsp"><img src=https://ehgt.org/g/mr.gif>')();return k.firstChild,k.style.setProperty("padding-bottom","0"),(C?0:void 0)!=null?k.style.setProperty("padding-top",C?0:void 0):k.style.removeProperty("padding-top"),k.addEventListener("click",P,!0),web.insert(k,web.createComponent(b,{id:""}),null),k})()},m),o.hotkeys&&hotkeysPageTurn(t);const R=async C=>{const P=await main.request(C,{errorText:helper.t("site.ehentai.fetch_img_page_source_failed")});$(P.responseText);const k=/nl\('(.+?)'\)/.exec(P.responseText)?.[1];if(!k)throw new Error(helper.t("site.ehentai.fetch_img_url_failed"));const N=new URL(C);return N.searchParams.set("nl",k),N.href},E=helper.singleThreaded(async(C,P)=>{const k=p.indexOf(P);if(k!==-1){if(p[k]=await I(s[k]),!await helper.testImgUrl(p[k])&&(s[k]=await R(s[k]),p[k]=await I(s[k]),main.toast.warn(helper.t("alert.retry_get_img_url",{i:k})),!await helper.testImgUrl(p[k])))return await helper.sleep(500),E(P);h("",k,p[k]);for(const N of Manga.imgList())if(N.loadType==="error")return E(N.src)}});d({title:helper.querySelector("#gj")?.textContent||helper.querySelector("#gn")?.textContent,onExit(C){C&&helper.scrollIntoView("#cdiv"),d("show",!1)},onImgError:E}),c("initialShow",o.autoShow)})().catch(t=>helper.log.error(t)),web.delegateEvents(["click"]);break}case"nhentai.net":{const t=require("solid-js/web"),o=require("main"),r=require("userscript/detectAd"),a=require("helper"),n={j:"jpg",p:"png",g:"gif",w:"webp",b:"bmp"};(async()=>{const{options:l,setFab:f,setManga:h,setComicLoad:c,showComic:d,comicMap:u,setComicMap:m}=await o.useInit("nhentai",{auto_page_turn:!0,block_totally:!0,open_link_new_page:!0,detect_ad:!0});if(Reflect.has(unsafeWindow,"gallery")){h({onExit(p){p&&a.scrollIntoView("#comment-container"),h("show",!1)}});const g=unsafeWindow._n_app.options.media_server;c(()=>_gallery.images.pages.map((p,s)=>`https://i${g}.nhentai.net/galleries/${_gallery.media_id}/${s+1}.${n[p.t]}`)),f("initialShow",l.autoShow);const b=(()=>{var p=t.template('<a href=javascript:; id=comicReadMode class="btn btn-secondary"><i class="fa fa-book"></i> Read')();return p.$$click=()=>d(),p})();document.getElementById("download").after(b),l.detect_ad&&a.querySelector("#tags .tag.tag-144644")&&(m("","adList",new o.ReactiveSet),await r.getAdPageByContent(a.querySelectorAll(".thumb-container img").map(p=>p.dataset.src),u[""].adList),a.createEffectOn(()=>u[""].imgList,p=>p?.length&&r.getAdPageByContent(p,u[""].adList)),a.useStyle(()=>u[""]?.adList?.size?[...u[""].adList].map(s=>`
            .thumb-container:nth-of-type(${s+1}):not(:hover) {
              filter: blur(8px);
              clip-path: border-box;
            }`).join(`
`):""));return}if(document.getElementsByClassName("gallery").length>0){if(l.open_link_new_page)for(const b of a.querySelectorAll('a:not([href^="javascript:"])'))b.setAttribute("target","_blank");const g=(unsafeWindow?._n_app??unsafeWindow?.n)?.options?.blacklisted_tags;if(g===void 0&&o.toast.error(a.t("site.nhentai.tag_blacklist_fetch_failed")),l.block_totally&&g?.length&&GM_addStyle(".blacklisted.gallery { display: none; }"),l.auto_page_turn){let b=a.querySelector("a.next")?.href;if(!b)return;GM_addStyle(`
      ...
      `);const w=new Set(g),p=document.getElementById("content"),s=()=>p.querySelector(":is(.index-container, #favcontainer):last-of-type"),y=a.singleThreaded(async()=>{if(!b)return;const $=await o.request(b,{fetch:!0,errorText:a.t("site.nhentai.fetch_next_page_failed")}),I=a.domParse($.responseText);history.replaceState(null,"",b);const L=I.querySelector(".index-container, #favcontainer");for(const A of L.querySelectorAll(".gallery")){for(const S of A.getElementsByTagName("img"))S.setAttribute("src",S.dataset.src);A.dataset.tags.split(" ").map(Number).some(S=>w.has(S))&&A.classList.add("blacklisted")}const M=I.querySelector(".pagination");b=M.querySelector("a.next")?.href,p.append(L,M);const x=document.createElement("hr");p.append(x),_.disconnect(),_.observe(s()),b||(x.style.animationPlayState="paused")},{abandon:!0});y();const _=new IntersectionObserver($=>$[0].isIntersecting&&y());_.observe(s()),a.querySelector("section.pagination")&&p.append(document.createElement("hr"))}}})().catch(l=>a.log.error(l)),t.delegateEvents(["click"]);break}case"yuri.website":{const t=require("main"),o=require("helper");(async()=>{const{options:r,setManga:a,setComicLoad:n,showComic:l,comicMap:f,needAutoShow:h}=await t.useInit("yurifans",{\u81EA\u52A8\u7B7E\u5230:!0});if(r.\u81EA\u52A8\u7B7E\u5230&&(async()=>{if(!globalThis.b2token)return;const c=new Date().toLocaleDateString("zh-CN");if(c!==localStorage.getItem("signDate"))try{const d=await t.request("/wp-json/b2/v1/userMission",{method:"POST",noTip:!0,headers:{Authorization:`Bearer ${b2token}`}}),u=JSON.parse(d.responseText);if(!(u?.mission?.date||!Number.isNaN(Number(u))))throw new Error("\u7B7E\u5230\u5931\u8D25");t.toast("\u81EA\u52A8\u7B7E\u5230\u6210\u529F"),localStorage.setItem("signDate",c)}catch{t.toast.error("\u81EA\u52A8\u7B7E\u5230\u5931\u8D25")}})(),!!o.querySelector('a.post-list-cat-item[title="\u5728\u7EBF\u533A-\u6F2B\u753B"]')){if(o.querySelector(".content-hidden")){const d=o.querySelector(".content-hidden").getElementsByTagName("img");await o.wait(()=>d.length,1e3)&&n(()=>[...d].map(u=>u.src));return}if(o.querySelector(".xControl")){h.val=!1;const c=async d=>{l(d),a({onPrev:Reflect.has(f,d-1)?()=>c(d-1):void 0,onNext:Reflect.has(f,d+1)?()=>c(d+1):void 0})};for(const[d,u]of o.querySelectorAll(".xControl > a").entries()){const m=u.parentElement.nextElementSibling;n(()=>[...m.getElementsByTagName("img")].map(g=>g.dataset.src??""),d),u.addEventListener("click",()=>m.getAttribute("style")!==""&&c(d))}return}await o.wait(()=>o.querySelectorAll(".entry-content img").length),n(()=>o.querySelectorAll(".entry-content img").map(c=>c.dataset.src||c.src))}})();break}case"mangacopy.com":case"copymanga.site":case"copymanga.info":case"copymanga.net":case"copymanga.org":case"copymanga.tv":case"copymanga.com":case"www.mangacopy.com":case"www.copymanga.site":case"www.copymanga.info":case"www.copymanga.net":case"www.copymanga.org":case"www.copymanga.tv":case"www.copymanga.com":{const t=require("solid-js/web"),o=require("solid-js"),r=require("main"),a=require("helper"),n={webp:"1",region:"1","User-Agent":"COPY/2.0.7|",version:"2.0.7",source:"copyApp",referer:"com.copymanga.app-2.0.7"},l=h=>{let c;const d=new CSSStyleSheet;document.adoptedStyleSheets.push(d);const u=async()=>{c||(async()=>{c=document.createElement("a");const w=await a.wait(()=>a.querySelector(".table-default-right"));c.target="_blank",w.insertBefore(c,w.firstElementChild);const p=document.createElement("span");p.textContent="\u6700\u5F8C\u95B1\u8B80\uFF1A",w.insertBefore(p,w.firstElementChild)})(),c.textContent="\u7372\u53D6\u4E2D",c.removeAttribute("href");const g=(await r.request(`/api/v3/comic2/${h}/query?platform=3`,{responseType:"json",fetch:!1,headers:n})).response?.results?.browse;if(!g){c.textContent=g===null?"\u7121":"\u672A\u8FD4\u56DE\u6578\u64DA";return}const b=g.chapter_id;if(!b){c.textContent="\u63A5\u53E3\u7570\u5E38";return}await d.replace(`ul a[href*="${b}"] {
        color: #fff !important;
        background: #1790E6;
      }`),c.href=`${window.location.pathname}/chapter/${b}`,c.textContent=g.chapter_name};setTimeout(u),document.addEventListener("visibilitychange",u)},f=async(h,c)=>{const{response:{results:d}}=await r.request(`/comicdetail/${h}/chapters`,{responseType:"json",errorText:"\u52A0\u8F09\u6F2B\u756B\u76EE\u9304\u5931\u6557",headers:n,fetch:!1}),m=await(async(s,y,_)=>{const $=await crypto.subtle.decrypt({name:"AES-CBC",iv:new TextEncoder().encode(_)},await crypto.subtle.importKey("raw",new TextEncoder().encode(y),{name:"AES-CBC"},!1,["decrypt"]),new Uint8Array(s.match(/.{1,2}/g).map(I=>Number.parseInt(I,16))).buffer);return JSON.parse(new TextDecoder().decode($))})(d.slice(16),unsafeWindow.dio||"xxxmanga.woo.key",d.slice(0,16));a.log(m);const{build:{type:g},groups:b}=m,w=s=>{const y=Object.fromEntries(g.map(({id:_})=>[_,[]]));for(const _ of s.chapters)y[_.type].push(_);switch(c){case"mobile":{for(const _ of a.querySelectorAll(".van-divider"))_.remove();return(()=>{var _=t.template('<div class="detailsTextContentTabs van-tabs van-tabs--line">')();return t.insert(_,t.createComponent(o.For,{each:g,children:({id:$,name:I})=>t.createComponent(o.Show,{get when(){return y[$].length},get children(){return[(()=>{var L=t.template('<div class=van-tabs__wrap><div role=tablist class="van-tabs__nav van-tabs__nav--line"><div role=tab class="van-tab van-tab--active"><span class="van-tab__text van-tab__text--ellipsis"><span></span></span></div><div class=van-tabs__line>')(),M=L.firstChild,x=M.firstChild,A=x.firstChild,v=A.firstChild,S=x.nextSibling;return M.style.setProperty("background","transparent"),t.insert(v,I),S.style.setProperty("width","0.24rem"),S.style.setProperty("transform","translateX(187.5px) translateX(-50%)"),S.style.setProperty("transition-duration","0.3s"),L})(),(()=>{var L=t.template('<div class=van-tab__pane><div class="chapterList van-grid">')(),M=L.firstChild;return M.style.setProperty("padding-left","0.24rem"),t.insert(M,t.createComponent(o.For,{get each(){return y[$]},children:x=>(()=>{var A=t.template('<div class="chapterItem oneLines van-grid-item"><a class="van-grid-item__content van-grid-item__content--center"><span class=van-grid-item__text>')(),v=A.firstChild,S=v.firstChild;return A.style.setProperty("flex-basis","25%"),A.style.setProperty("padding-right","0.24rem"),A.style.setProperty("margin-top","0.24rem"),t.insert(S,()=>x.name),t.effect(T=>{var R=s.last_chapter.uuid===x.id,E=`/comic/${h}/chapter/${x.id}`;return R!==T.e&&A.classList.toggle("red",T.e=R),E!==T.t&&t.setAttribute(v,"href",T.t=E),T},{e:void 0,t:void 0}),A})()})),L})()]}})})),_})()}case"web":return[(()=>{var _=t.template("<span>")();return t.insert(_,()=>s.name),_})(),(()=>{var _=t.template('<div class=table-default><div class=table-default-title><ul class="nav nav-tabs"role=tablist></ul><div class=table-default-right><span>\u66F4\u65B0\u5167\u5BB9\uFF1A</span><a target=_blank></a><span>\u66F4\u65B0\u6642\u9593\uFF1A</span><span></span></div></div><div class=table-default-box><div class=tab-content>')(),$=_.firstChild,I=$.firstChild,L=I.nextSibling,M=L.firstChild,x=M.nextSibling,A=x.nextSibling,v=A.nextSibling,S=$.nextSibling,T=S.firstChild;return t.insert(I,t.createComponent(o.For,{each:g,children:({id:R,name:E})=>(()=>{var C=t.template("<li class=nav-item><a class=nav-link data-toggle=tab role=tab aria-selected=false>")(),P=C.firstChild;return t.insert(P,E),t.effect(k=>{var N=y[R].length===0,O=`#${s.path_word}${E}`;return N!==k.e&&P.classList.toggle("disabled",k.e=N),O!==k.t&&t.setAttribute(P,"href",k.t=O),k},{e:void 0,t:void 0}),C})()})),t.insert(x,()=>s.last_chapter.name),t.insert(v,()=>s.last_chapter.datetime_created),t.insert(T,t.createComponent(o.For,{each:g,children:({id:R,name:E})=>(()=>{var C=t.template('<div role=tabpanel class="tab-pane fade"><ul>')(),P=C.firstChild;return t.insert(P,t.createComponent(o.For,{get each(){return y[R]},children:k=>(()=>{var N=t.template("<a target=_blank><li>")(),O=N.firstChild;return N.style.setProperty("display","block"),t.insert(O,()=>k.name),t.effect(D=>{var q=`/comic/${h}/chapter/${k.id}`,z=k.name;return q!==D.e&&t.setAttribute(N,"href",D.e=q),z!==D.t&&t.setAttribute(N,"title",D.t=z),D},{e:void 0,t:void 0}),N})()})),t.effect(()=>t.setAttribute(C,"id",`${s.path_word}${E}`)),C})()})),t.effect(()=>t.setAttribute(x,"href",`/comic/${h}/chapter/${s.last_chapter.comic_id}`)),_})()];default:return t.createComponent(o.For,{each:g,children:({id:_,name:$})=>t.createComponent(o.Show,{get when(){return y[_].length},get children(){var I=t.template("<div class=card><div class=card-body><h2 class=card-title></h2><ul>")(),L=I.firstChild,M=L.firstChild,x=M.nextSibling;return I.style.setProperty("max-width","100em"),I.style.setProperty("margin","1em auto"),t.insert(M,$),t.insert(x,t.createComponent(o.For,{get each(){return y[_]},children:A=>(()=>{var v=t.template('<a class="btn btn-outline-primary">')();return t.insert(v,()=>A.name),t.effect(S=>{var T=s.last_chapter.uuid===A.id,R=`/comic/${h}/chapter/${A.id}`;return T!==S.e&&v.classList.toggle("active",S.e=T),R!==S.t&&t.setAttribute(v,"href",S.t=R),S},{e:void 0,t:void 0}),v})()})),I}})})}};let p;switch(c){case"mobile":p=a.querySelector(".detailsTextContent");for(const s of a.querySelectorAll("button.van-dialog__confirm"))s.click();break;case"web":p=a.querySelector(".upLoop");break;default:p=a.querySelector("main"),p.textContent="",GM_addStyle("ul .btn { height: fit-content; width: fit-content; margin: 1em; }");break}t.render(()=>t.createComponent(o.For,{get each(){return Object.values(b)},children:w}),p);for(const s of a.querySelectorAll(".upLoop .table-default-title"))s.querySelector(".nav-link:not(.disabled)")?.click()};(async()=>{const h=document.cookie.split("; ").find(u=>u.startsWith("token="))?.replace("token=","");h&&Reflect.set(n,"Authorization",`Token ${h}`);let c="",d="";if(window.location.href.includes("/chapter/")?[,,c,,d]=window.location.pathname.split("/"):window.location.href.includes("/comicContent/")&&([,,,c,d]=window.location.pathname.split("/")),c&&d){const{setComicLoad:u,setManga:m}=await r.useInit("copymanga"),g=a.querySelector("main .img+.title");g&&(g.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u4F60\u8A2A\u554F\u7684\u5167\u5BB9\u66AB\u4E0D\u5B58\u5728\uFF0C\u8ACB\u9EDE\u9078\u53F3\u4E0B\u89D2\u6309\u9215\u5617\u8A66\u52A0\u8F09\u6F2B\u756B"),u(async()=>{g&&(g.textContent="\u6F2B\u756B\u52A0\u8F09\u4E2D\uFF0C\u8ACB\u5750\u548C\u653E\u5BEC");const w=await r.request(`/api/v3/comic/${c}/chapter2/${d}?platform=3`,{responseType:"json",headers:n,noCheckCode:!0});if(w.status!==200){const _=`\u6F2B\u756B\u52A0\u8F09\u5931\u6557\uFF1A${w.response.message||w.status}`;throw g&&(g.textContent=_),new Error(_)}if(g){g.textContent="\u6F2B\u756B\u52A0\u8F09\u6210\u529F\u{1F973}";const{chapter:{name:_},comic:{name:$}}=w.response.results;document.title=`${$} - ${_} - \u62F7\u8C9D\u6F2B\u756B \u62F7\u8D1D\u6F2B\u753B`}if(g??!a.querySelector(".comicContent-next")){const{chapter:{next:_,prev:$}}=w.response.results;m({onNext:_?()=>window.location.assign(`/comic/${c}/chapter/${_}`):void 0,onPrev:$?()=>window.location.assign(`/comic/${c}/chapter/${$}`):void 0})}else m({onNext:a.querySelectorClick(".comicContent-next a:not(.prev-null)"),onPrev:a.querySelectorClick(".comicContent-prev:not(.index,.list) a:not(.prev-null)")});const p=[],{words:s,contents:y}=w.response.results.chapter;for(let _=0;_<y.length;_++)p[s[_]]=y[_].url.replace(/(?<=.*(\/|\.))c800x/,"c1500x");return p}),m({commentList:await(async()=>{const w=window.location.pathname.split("/").at(-1);return(await r.request(`/api/v3/roasts?chapter_id=${w}&limit=100&offset=0&_update=true`,{responseType:"json",errorText:"\u83B7\u53D6\u6F2B\u753B\u8BC4\u8BBA\u5931\u8D25"})).response.results.list.map(({comment:s})=>s)})()});return}if(!d&&window.location.href.includes("/comic/")){if(c=window.location.href.split("/comic/")[1],!c)return;let u;const m=window.location.href.includes("/h5/");if(document.title==="404 - \u62F7\u8C9D\u6F2B\u756B"?u=m?"mobile":"404":m?(await a.wait(()=>a.querySelector(".van-toast__text")?.parentElement?.style.display==="none"),u=await a.wait(()=>{if(a.querySelector(".isBan")?.textContent?.includes("\u4E0D\u63D0\u4F9B\u95B1\u89BD"))return"mobile";const g=a.querySelector(".van-dialog__message");if(g?.textContent?.includes("\u6F2B\u756B\u672A\u627E\u5230")){g.textContent=`\u6F2B\u756B\u672A\u627E\u5230!
\u8ACB\u5750\u548C\u653E\u5BEC\uFF0C\u7B49\u5F85\u76EE\u9304\u751F\u6210`;for(const b of a.querySelectorAll(".detailsTextContentTabs"))b.remove();return"mobile"}},1e3)):(a.querySelector(".wargin")?.textContent?.includes("\u4E0D\u63D0\u4F9B\u95B1\u89BD")||!await a.wait(()=>a.querySelector(".upLoop .table-default-title"),1e3))&&(u=a.querySelector(".comicParticulars-title")?"web":"404"),u){const g=a.querySelector(".isBan, .wargin");g&&(g.style.textDecoration="line-through");const b=a.querySelector("main .img+.title");b&&(b.textContent="ComicRead \u63D0\u793A\u60A8\uFF1A\u4F60\u8A2A\u554F\u7684\u5167\u5BB9\u66AB\u4E0D\u5B58\u5728\uFF0C\u8ACB\u5750\u548C\u653E\u5BEC\uFF0C\u7B49\u5F85\u76EE\u9304\u751F\u6210"),await f(c,u)}!m&&h&&l(c)}})();break}case"www.pixiv.net":{let t=[];options={name:"pixiv",getImgList:()=>t,SPA:{async isMangaPage(){const o=Number(window.location.pathname.split("/")[2]);return!o||!window.location.pathname.startsWith("/artworks/")?(t.length=0,!1):(t=(await main.request(`/ajax/illust/${o}/pages`,{responseType:"json"})).response.body.map(a=>a.urls.original),t.length>1)}},initOptions:{autoShow:!1,defaultOption:{pageNum:1}}};break}case"www.ponpomu.com":{options={name:"terraHistoricus",wait:()=>!!helper.querySelector(".comic-page-container img"),getImgList:()=>helper.querySelectorAll(".comic-page-container img").map(t=>t.dataset.srcset),SPA:{isMangaPage:()=>window.location.href.includes("/comic/"),getOnPrev:()=>helper.querySelectorClick(".prev-btn:not(.invisible) a"),getOnNext:()=>helper.querySelectorClick(".next-btn:not(.invisible) a")}};break}case"manhua.zaimanhua.com":{options={name:"zaiManHua",wait:()=>!!helper.querySelector(".scrollbar-demo-item"),getImgList:()=>unsafeWindow.__NUXT__.data.getChapters?.data?.chapterInfo?.page_url,SPA:{isMangaPage:()=>window.location.pathname.startsWith("/view/"),getOnNext:()=>helper.querySelectorClick("#prev_chapter"),getOnPrev:()=>helper.querySelectorClick("#next_chapter")}};break}case"m.zaimanhua.com":{const t=async(r,a)=>{const n=await main.request(`https://v4api.zaimanhua.com/app/v1/comic/chapter/${r}/${a}`,{responseType:"json"});return n.response.errno&&main.toast.error(`${helper.t("alert.comic_load_error")}: ${n.response.errmsg}`,{throw:!0}),n.response.data.data},o=async r=>{const a=await main.request(`https://v4api.zaimanhua.com/app/v1/comic/detail/${r}`,{responseType:"json"});return a.response.errno&&main.toast.error(`${helper.t("alert.comic_load_error")}: ${a.response.errmsg}`,{throw:!0}),a.response.data.data};options={name:"zaiManHua",async getImgList({setManga:r}){const a=new URLSearchParams(window.location.search),n=Number(a.get("comic_id")),l=Number(a.get("chapter_id"));if(!n||!l)throw new Error(helper.t("site.changed_load_failed"));const f=await o(n),h=(f.chapters.length===1?f.chapters[0]:f.chapters.find(u=>u.data.find(m=>m.chapter_id===l))).data;h.sort((u,m)=>u.chapter_order-m.chapter_order);const c=h.findIndex(u=>u.chapter_id===l);return r({onPrev:c>0?()=>window.location.assign(`/pages/comic/page?comic_id=${n}&chapter_id=${h[c-1].chapter_id}`):void 0,onNext:c+1<h.length?()=>window.location.assign(`/pages/comic/page?comic_id=${n}&chapter_id=${h[c+1].chapter_id}`):void 0}),(await t(n,l)).page_url_hd},SPA:{isMangaPage:()=>window.location.pathname==="/pages/comic/page"}};break}case"terra-historicus.hypergryph.com":{const t=()=>`https://terra-historicus.hypergryph.com/api${window.location.pathname}`,o=r=>async()=>{const a=await main.request(`${t()}/page?pageNum=${r+1}`);return JSON.parse(a.responseText).data.url};options={name:"terraHistoricus",wait:()=>!!helper.querySelector(".HG_COMIC_READER_main"),async getImgList(){const a=(await main.request(t(),{responseType:"json"})).response.data.pageInfos;if(a.length===0&&window.location.pathname.includes("episode"))throw new Error("\u83B7\u53D6\u56FE\u7247\u5217\u8868\u65F6\u51FA\u9519");return helper.plimit(helper.createSequence(a.length).map(o))},SPA:{isMangaPage:()=>window.location.href.includes("episode"),getOnPrev:()=>helper.querySelectorClick("footer .HG_COMIC_READER_prev a"),getOnNext:()=>helper.querySelectorClick("footer .HG_COMIC_READER_prev+.HG_COMIC_READER_buttonEp a")}};break}case"jmcomic-zzz.one":case"jmcomic-zzz.org":case"18comic-rajang.vip":case"18comic-rajang.club":case"18comic-mhws.cc":case"18comic.org":case"18comic.vip":{const t=require("main"),o=require("helper");(async()=>{if(!window.location.pathname.includes("/photo/"))return;const{setComicLoad:r,setManga:a,dynamicLoad:n}=await t.useInit("jm");for(;!unsafeWindow?.onImageLoaded;){if(document.readyState==="complete"){t.toast.error("\u65E0\u6CD5\u83B7\u53D6\u56FE\u7247",{duration:Number.POSITIVE_INFINITY});return}await o.sleep(100)}a({onPrev:o.querySelectorClick(()=>o.querySelector(".menu-bolock-ul .fa-angle-double-left")?.parentElement),onNext:o.querySelectorClick(()=>o.querySelector(".menu-bolock-ul .fa-angle-double-right")?.parentElement)});const l=o.querySelectorAll(".scramble-page:not(.thewayhome) > img");if(unsafeWindow.aid<unsafeWindow.scramble_id||unsafeWindow.speed==="1")return r(()=>l.map(d=>d.dataset.original??""));const f=async d=>{try{return await t.request(d,{responseType:"blob",fetch:!0,noTip:!0},3)}catch{return await t.request(d,{responseType:"blob",revalidate:!0,fetch:!1},3)}},h=async d=>{if(d.src.startsWith("blob:"))return d.src;const u=d.dataset.original,m=await f(d.dataset.original);if(m.response.size===0)return t.toast.warn(`\u4E0B\u8F7D\u539F\u56FE\u65F6\u51FA\u9519: ${d.dataset.page}`),"";d.src=`${URL.createObjectURL(m.response)}#${d.src}`;try{await o.waitImgLoad(d,1e3*10)}catch{return URL.revokeObjectURL(d.src),d.src=u,t.toast.warn(`\u52A0\u8F7D\u539F\u56FE\u65F6\u51FA\u9519: ${d.dataset.page}`),""}try{d.nextElementSibling?.tagName==="CANVAS"&&d.nextElementSibling.remove(),unsafeWindow.onImageLoaded(d);const g=await o.canvasToBlob(d.nextElementSibling,"image/webp",1);if(URL.revokeObjectURL(d.src),!g)throw new Error("\u8F6C\u6362\u56FE\u7247\u65F6\u51FA\u9519");return`${URL.createObjectURL(g)}#.webp`}catch(g){return d.src=u,t.toast.warn(`\u8F6C\u6362\u56FE\u7247\u65F6\u51FA\u9519: ${d.dataset.page}, ${g.message}`),""}};await o.wait(()=>{const d=o.querySelectorAll(".lazy-loaded").length;return d>0&&o.querySelectorAll("canvas").length-d<=1}),r(n(async d=>o.plimit(l.map((u,m)=>async()=>{d(m,await h(u))})),l.length))})().catch(r=>o.log.error(r));break}case"tw.manhuagui.com":case"m.manhuagui.com":case"www.mhgui.com":case"www.manhuagui.com":{if(!/\/comic\/\d+\/\d+\.html/.test(window.location.pathname))break;let comicInfo;try{const dataScript=helper.querySelectorAll("body > script:not([src])").find(t=>t.innerHTML.startsWith("window["));if(!dataScript)throw new Error(helper.t("site.changed_load_failed"));comicInfo=JSON.parse(eval(dataScript.innerHTML.slice(26)).match(/(?<=.*?\(){.+}/)[0])}catch{main.toast.error(helper.t("site.changed_load_failed"));break}GM_addStyle("#smh-msg-box { z-index: 2147483647 !important }");const handlePrevNext=t=>{if(t===0)return;const o=window.location.pathname.replace(/(?<=\/)\d+(?=\.html)/,`${t}`);return()=>window.location.assign(o)};options={name:"manhuagui",getImgList(){const t=Object.entries(comicInfo.sl).map(o=>`${o[0]}=${o[1]}`).join("&");if(comicInfo.files)return comicInfo.files.map(o=>`${unsafeWindow.pVars.manga.filePath}${o}?${t}`);if(comicInfo.images){const{origin:o}=new URL(helper.querySelector("#manga img").src);return comicInfo.images.map(r=>`${o}${r}?${t}`)}return main.toast.error(helper.t("site.changed_load_failed"),{throw:!0}),[]},onNext:handlePrevNext(comicInfo.nextId),onPrev:handlePrevNext(comicInfo.prevId)};break}case"www.manhuadb.com":{if(!Reflect.has(unsafeWindow,"img_data_arr"))break;options={name:"manhuaDB",getImgList:()=>unsafeWindow.img_data_arr.map(t=>`${unsafeWindow.img_host}/${unsafeWindow.img_pre}/${t.img}`),onPrev:()=>unsafeWindow.goNumPage("pre"),onNext:()=>unsafeWindow.goNumPage("next")};break}case"www.manhuaren.com":case"m.1kkk.com":case"www.1kkk.com":case"tel.dm5.com":case"en.dm5.com":case"cnc.dm5.com":case"www.dm5.cn":case"www.dm5.com":{if(!Reflect.has(unsafeWindow,"DM5_CID"))break;const imgNum=unsafeWindow.DM5_IMAGE_COUNT??unsafeWindow.imgsLen;if(!(Number.isSafeInteger(imgNum)&&imgNum>0)){main.toast.error(helper.t("site.changed_load_failed"));break}const getPageImg=async i=>{const res=await unsafeWindow.$.ajax({type:"GET",url:"chapterfun.ashx",data:{cid:unsafeWindow.DM5_CID,page:i,key:unsafeWindow.$("#dm5_key").length>0?unsafeWindow.$("#dm5_key").val():"",language:1,gtk:6,_cid:unsafeWindow.DM5_CID,_mid:unsafeWindow.DM5_MID,_dt:unsafeWindow.DM5_VIEWSIGN_DT,_sign:unsafeWindow.DM5_VIEWSIGN}});return eval(res)},handlePrevNext=(t,o)=>helper.querySelectorClick(()=>helper.querySelector(t)??helper.querySelectorAll(".view-bottom-bar a").find(r=>r.textContent?.includes(o)));options={name:"dm5",getImgList({dynamicLoad:t}){return Array.isArray(unsafeWindow.newImgs)&&unsafeWindow.newImgs.every(helper.isUrl)?unsafeWindow.newImgs:t(async o=>{const r=new Set;for(;r.size<imgNum;)for(const a of await getPageImg(r.size+1))r.has(a)||(r.add(a),o(r.size-1,a))},imgNum)()},onPrev:handlePrevNext(".logo_1","\u4E0A\u4E00\u7AE0"),onNext:handlePrevNext(".logo_2","\u4E0B\u4E00\u7AE0"),onExit:t=>t&&helper.scrollIntoView(".postlist")};break}case"www.wnacg01.cc":case"www.wn03.ru":case"www.wnacg.com":case"wnacg.com":{const t=helper.querySelector("#bodywrap a.btn");if(t&&(t.style.setProperty("background-color","#607d8b"),t.style.setProperty("background-image","none")),!Reflect.has(unsafeWindow,"imglist"))break;options={name:"wnacg",getImgList:()=>unsafeWindow.imglist.filter(({caption:o})=>o!=="\u559C\u6B61\u7D33\u58EB\u6F2B\u756B\u7684\u540C\u5B78\u8ACB\u52A0\u5165\u6536\u85CF\u54E6\uFF01").map(({url:o})=>new URL(o,window.location.origin).href)};break}case"www.mangabz.com":case"mangabz.com":{if(!Reflect.has(unsafeWindow,"MANGABZ_CID"))break;const imgNum=unsafeWindow.MANGABZ_IMAGE_COUNT??unsafeWindow.imgsLen;if(!(Number.isSafeInteger(imgNum)&&imgNum>0)){main.toast.error(helper.t("site.changed_load_failed"));break}const getPageImg=async i=>{const res=await unsafeWindow.$.ajax({type:"GET",url:"chapterimage.ashx",data:{cid:unsafeWindow.MANGABZ_CID,page:i,key:"",_cid:unsafeWindow.MANGABZ_CID,_mid:unsafeWindow.MANGABZ_MID,_dt:unsafeWindow.MANGABZ_VIEWSIGN_DT,_sign:unsafeWindow.MANGABZ_VIEWSIGN}});return eval(res)},handlePrevNext=(t,o)=>helper.querySelectorClick(()=>helper.querySelector(t)??helper.querySelectorAll(".bottom-bar-tool a").find(r=>r.textContent?.includes(o)));options={name:"mangabz",getImgList:({dynamicLoad:t})=>t(async o=>{const r=new Set;for(;r.size<imgNum;)for(const a of await getPageImg(r.size+1))r.has(a)||(r.add(a),o(r.size-1,a))},imgNum)(),onNext:handlePrevNext('body > .container a[href^="/"]:last-child',"\u4E0B\u4E00"),onPrev:handlePrevNext('body > .container a[href^="/"]:first-child',"\u4E0A\u4E00")};break}case"komiic.com":{const t=`
        ...
        }`,o=async()=>{const a=/chapter\/(\d+)/.exec(window.location.pathname)?.[1];if(!a)throw new Error(helper.t("site.changed_load_failed"));return(await main.request("/api/query",{method:"POST",responseType:"json",headers:{"content-type":"application/json"},data:JSON.stringify({operationName:"imagesByChapterId",variables:{chapterId:`${a}`},query:t})})).response.data.imagesByChapterId.map(({kid:l})=>`https://komiic.com/api/image/${l}`)},r=a=>async()=>(await helper.waitDom(".v-bottom-navigation__content"),helper.querySelectorClick(".v-bottom-navigation__content > button:not([disabled])",a));options={name:"komiic",getImgList:o,SPA:{isMangaPage:()=>/comic\/\d+\/chapter\/\d+\/images\//.test(window.location.href),getOnPrev:r("\u4E0A\u4E00"),getOnNext:r("\u4E0B\u4E00")}};break}case"mangadex.org":{options={name:"mangadex",async getImgList(){const t=window.location.pathname.split("/").at(2),{response:{baseUrl:o,chapter:{data:r,hash:a}}}=await main.request(`https://api.mangadex.org/at-home/server/${t}?forcePort443=false`,{responseType:"json"});return r.map(n=>o+"/data/"+a+"/"+n)},SPA:{isMangaPage:()=>/^\/chapter\/.+/.test(window.location.pathname),getOnPrev:()=>helper.querySelectorClick('#chapter-selector > a[href^="/chapter/"]:nth-of-type(1)'),getOnNext:()=>helper.querySelectorClick('#chapter-selector > a[href^="/chapter/"]:nth-of-type(2)'),handleUrl:t=>t.href.replace(/(?<=\/chapter\/.+?)\/.*/,"")}};break}case"noy1.top":{options={name:"NoyAcg",async getImgList(){const[,,t]=window.location.hash.split("/"),r=(await helper.wait(()=>helper.querySelector(".lazy-load-image-background img"))).src.split(t)[0],a=await helper.wait(()=>helper.querySelectorAll(".lazy-load-image-background").length);return helper.range(a,n=>`${r}${t}/${n+1}.webp`)},SPA:{isMangaPage:()=>window.location.hash.startsWith("#/read/")}};break}case"8.twobili.com":case"a.twobili.com":case"articles.onemoreplace.tw":case"www.comicabc.com":{if(!["/online/","/ReadComic/","/comic/"].some(r=>location.pathname.startsWith(r)))break;options={name:"8comic",getImgList:()=>[...unsafeWindow.xx.matchAll(/(?<= s=").+?(?=")/g)].map(([r])=>decodeURIComponent(r)),onNext:helper.querySelectorClick("#nextvol"),onPrev:helper.querySelectorClick("#prevvol")};break}case"m.77mh.me":case"www.77mh.me":case"m.77mh.xyz":case"www.77mh.xyz":case"m.77mh.nl":case"www.77mh.nl":{if(!Reflect.has(unsafeWindow,"msg"))break;options={name:"77mh",async getImgList(){const t=unsafeWindow.img_qianz??unsafeWindow.ImgSvrList;return unsafeWindow.msg.split("|").map(o=>`${t}${o}`)},onNext:helper.querySelectorClick("#pnpage > a","\u4E0B\u4E00"),onPrev:helper.querySelectorClick("#pnpage > a","\u4E0A\u4E00")};break}case"www.relamanhua.org":case"www.manga2024.com":case"www.2024manga.com":{if(!window.location.pathname.includes("/chapter/")&&!document.querySelector(".disData[contentkey]"))break;options={name:"relamanhua",getImgList:async()=>{const[,,o,,r]=window.location.pathname.split("/");return(await main.request(`https://mapi.fgjfghkk.club/api/v3/comic/${o}/chapter/${r}?platform=1&_update=true`,{responseType:"json"})).response.results.chapter.contents.map(({url:n})=>n.replace(".h800x.",".h1500x."))},onNext:helper.querySelectorClick(".comicContent-next a:not(.prev-null)"),onPrev:helper.querySelectorClick(".comicContent-prev:not(.index,.list) a:not(.prev-null)")};break}case"hitomi.la":{options={name:"hitomi",wait:()=>unsafeWindow.galleryinfo&&Reflect.has(unsafeWindow.galleryinfo,"files")&&unsafeWindow.galleryinfo.type!=="anime",getImgList:()=>(unsafeWindow.galleryinfo?.files).map(t=>unsafeWindow.url_from_url_from_hash(unsafeWindow.galleryinfo.id,t,"webp"))};break}case"shupogaki.moe":case"hoshino.one":case"niyaniya.moe":{const t=async a=>new Promise(n=>{const l=new XMLHttpRequest;l.responseType="blob",l.open("GET",a),l.onload=()=>{n(URL.createObjectURL(l.response))},l.send()}),o=()=>window.location.href.includes("/g/"),r=localStorage.getItem("clearance");options={name:"schale",async getImgList({dynamicLoad:a}){const[,,n,l]=window.location.pathname.split("/"),f=await main.request(`https://api.schale.network/books/detail/${n}/${l}?crt=${r}`,{fetch:!0,responseType:"json",method:"POST"}),[[h,{id:c,key:d}]]=Object.entries(f.response.data).filter(([,w])=>w.id&&w.key).sort(([,w],[,p])=>p.size-w.size),u=await main.request(`https://api.schale.network/books/data/${n}/${l}/${c}/${d}/${h}?crt=${r}`,{fetch:!0,responseType:"json"}),{base:m,entries:g}=u.response,b=g.length;return a(async w=>{for(const[p,{path:s,dimensions:y}]of g.entries()){if(!o)break;const _=performance.now();w(p,await t(`${m}${s}?w=${y[0]}`)),await helper.sleep(500-(performance.now()-_))}},b)()},SPA:{isMangaPage:o}};break}case"kemono.su":case"kemono.party":{const t=require("main"),o=require("helper");(async()=>{const r=()=>location.pathname.includes("/post/");await o.waitUrlChange(r);const{options:a,setComicLoad:n,showComic:l,switchComic:f,needAutoShow:h,setFab:c,setManga:d,setComicMap:u}=await t.useInit("kemono",{autoShow:!1,defaultOption:{pageNum:1},load_original_image:!0});n(()=>o.querySelectorAll(".post__thumbnail a").map(g=>g.href),"original"),n(()=>o.querySelectorAll(".post__thumbnail img").map(g=>g.src),"thumbnail"),o.createEffectOn(()=>a.load_original_image,(g,b)=>{if(!b)return f(g?"original":"thumbnail");h.val=a.autoShow,l(g?"original":"thumbnail")});const m=new Set(["zip","rar","7z","cbz","cbr","cb7"]);for(const g of o.querySelectorAll(".post__attachment a")){if(!m.has(g.href.split(".").pop()))continue;const b=document.createElement("a");b.href=`https://comic-read.pages.dev/?url=${encodeURIComponent(g.href)}`,b.textContent=g.textContent.replace("Download ","ComicReadPWA - "),b.className=g.className,b.style.opacity=".6",g.parentNode.insertBefore(b,g.nextElementSibling)}o.onUrlChange(async()=>{if(!r()){c("show",!1),d({show:!1}),u("original","imgList",void 0),u("thumbnail","imgList",void 0);return}c("show",void 0),d({onPrev:void 0,onNext:void 0}),h.val=a.autoShow,u("","imgList",void 0),h.val&&a.autoShow&&await l("")})})();break}case"nekohouse.su":{options={name:"nekohouse",getImgList:()=>helper.querySelectorAll(".fileThumb").map(t=>t.getAttribute("href")),initOptions:{autoShow:!1,defaultOption:{pageNum:1}}};break}case"nicomanga.com":case"weloma.art":case"welovemanga.one":{if(!helper.querySelector("#listImgs, .chapter-content"))break;const t=async()=>{const o=helper.querySelectorAll("img.chapter-img:not(.ls-is-cached)").map(r=>(r.dataset.src||r.dataset.srcset||r.dataset.original||r.src).trim()).filter(Boolean);return o.length>0&&o.every(r=>!/loading.*\.gif/.test(r))?o:(await helper.sleep(500),t())};options={name:"welovemanga",getImgList:t,onNext:helper.querySelectorClick(".rd_top-right.next:not(.disabled)"),onPrev:helper.querySelectorClick(".rd_top-left.prev:not(.disabled)")};break}case"comic-read.pages.dev":{unsafeWindow.GM_xmlhttpRequest=GM_xmlhttpRequest,unsafeWindow.toast=main.toast;break}default:if(document.querySelector(`head > meta[content="A manga reader that runs tachiyomi's extensions"]`)){const t=(r,a)=>{window.location.pathname=`/manga/${r}/chapter/${a}`},o=async(r,a)=>{const n=await main.request("/api/graphql",{method:"POST",data:JSON.stringify({operationName:"GET_CHAPTERS",query:`query GET_CHAPTERS($mangaId: Int!, $chapterId: Int!) {
                chapters(condition: {
                  mangaId: $mangaId, sourceOrder: $chapterId}
                ) { nodes { pageCount } }
                manga(id: $mangaId) { chapters { totalCount } }
              }`,variables:{mangaId:r,chapterId:a}}),responseType:"json"});return n.response.data.chapters.nodes[0].pageCount<=0?(await helper.sleep(200),o(r,a)):n.response.data};options={name:"Tachidesk",SPA:{isMangaPage:()=>/\/manga\/\d+\/chapter\/\d+/.test(window.location.pathname)},async getImgList({setManga:r}){const[,,a,,n]=window.location.pathname.split("/").map(Number),l=await o(a,n),{pageCount:f}=l.chapters.nodes[0],h=l.manga.chapters.totalCount;return r({onPrev:n>0?()=>t(a,n-1):void 0,onNext:n<h?()=>t(a,n+1):void 0}),helper.range(f,c=>`/api/v1/manga/${a}/chapter/${n}/page/${c}`)},onShowImgsChange:helper.debounce((r,a)=>{const n=a[[...r].at(-1)].src;helper.querySelector(`img[src$="${n}"]`)?.scrollIntoView({behavior:"instant",block:"end"})},500)}}else(async()=>{if(await GM.getValue(window.location.hostname)!==void 0)return helper.requestIdleCallback(otherSite.otherSite);await GM.registerMenuCommand((t=>{switch(t){case"en":return"Enter simple reading mode";case"ru":return"\u0412\u043A\u043B\u044E\u0447\u0438\u0442\u044C \u043F\u0440\u043E\u0441\u0442\u043E\u0439 \u0440\u0435\u0436\u0438\u043C \u0447\u0442\u0435\u043D\u0438\u044F";case"ta":return"\u0B8E\u0BB3\u0BBF\u0BAF \u0BB5\u0BBE\u0B9A\u0BBF\u0BAA\u0BCD\u0BAA\u0BC1 \u0BAA\u0BAF\u0BA9\u0BCD\u0BAE\u0BC1\u0BB1\u0BC8\u0BAF\u0BC8\u0BAA\u0BCD \u0BAA\u0BAF\u0BA9\u0BCD\u0BAA\u0B9F\u0BC1\u0BA4\u0BCD\u0BA4\u0BB5\u0BC1\u0BAE\u0BCD";default:return"\u4F7F\u7528\u7B80\u6613\u9605\u8BFB\u6A21\u5F0F"}})(await languages.getInitLang()),otherSite.otherSite)})()}options&&main.universal(options)}catch(t){helper.log.error(t)}
