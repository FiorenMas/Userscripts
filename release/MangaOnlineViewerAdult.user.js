...
`,Le=`<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-autofit-height" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
...
`,Ae=`<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-arrow-autofit-width" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
...
`,An=`<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-eye" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
...
`;async function gi(e){if(e.waitAttr!==void 0){l(`Waiting for Attribute ${e.waitAttr[1]} of ${e.waitAttr[0]}`);const t=await be(e.waitAttr[0],e.waitAttr[1]);l(`Found Attribute ${e.waitAttr[1]} of ${e.waitAttr[0]} = ${t}`)}}async function pi(e){if(e.waitEle!==void 0){l(`Waiting for Element ${e.waitEle}`);const t=await Lt(e.waitEle);l(`Found Element ${e.waitEle} = `,t)}}async function hi(e){if(e.waitVar!==void 0){l(`Waiting for Variable ${e.waitVar}`);const t=await fe(e.waitVar);l(`Found Variable ${e.waitVar} = ${t}`)}}async function wi(e){if(e.waitFunc!==void 0){l(`Waiting to pass Function check ${e.waitFunc}`);const t=await we(e.waitFunc);l(`Found Function check ${e.waitFunc} = ${t}`)}}async function bi(e){e.waitTime!==void 0&&(l(`Waiting to for ${e.waitTime} milliseconds`),await new Promise(t=>{setTimeout(t,e.waitTime)}),l("Continuing after timer"))}const fi={url:/(file:\/\/\/.+(index)?.html)/},yi=["image/apng","image/bmp","image/gif","image/jpeg","image/pjpeg","image/png","image/svg+xml","image/tiff","image/webp","image/x-icon"],vi=/.(png|jpg|jpeg|gif|bmp|webp)$/i,vt=(e,t)=>e.localeCompare(t,navigator.languages[0]||navigator.language,{numeric:!0,ignorePunctuation:!0});function Ei(e){return yi.includes(e.type)}const ki=e=>{const t=new Uint8Array(e),n=new Blob([t.buffer]);return URL.createObjectURL(n)};async function Si(e){const t=await JSZip.loadAsync(e),n=t.filter((o,r)=>!r.dir&&vi.test(r.name)).sort((o,r)=>vt(o.name,r.name));return l("Files in zip:",t.files),Promise.all(n.map(o=>o.async("arraybuffer").then(ki)))}function Et(e,t){$({title:e,series:"?reload",pages:t.length,begin:1,prev:"#",next:"#",lazy:!1,listImages:t}).then(()=>l("Page loaded"))}async function xi(e){const t=await Si(e);Et(typeof e=="string"?e:e.name,t)}function kt(e){const t=e.target,n=Array.from(t.files).filter(Ei).sort((o,r)=>vt(o.webkitRelativePath||o.name,r.webkitRelativePath||r.name));l("Local Files: ",n,n.map(o=>o.webkitRelativePath||o.name)),t.files?.[0]&&Et(t.files[0].webkitRelativePath.split("/")[0]||"Local Images",n.map(URL.createObjectURL))}function Mi(){return fi.url.test(window.location.href)?(document.querySelector("#MangaOnlineViewer, #LocalTest")&&(document.querySelector("#LocalTest")?.setAttribute("style","display:none"),document.querySelector("#file")?.addEventListener("change",e=>{const t=e.target;t.files?.[0]&&xi(t.files[0])}),document.querySelector("#folder")?.addEventListener("change",kt),document.querySelector("#images")?.addEventListener("change",kt),l("Waiting for zip/images upload")),!0):!1}function Oi(e,t,n){let o=e;return Number.isNaN(o)||o<n.min()?o=n.min():o>n.max()?o=n.max():o>t&&(o=t),o}function Ii(e,t,n){let o=e;return Number.isNaN(o)||o>n.max()?o=n.max():o<n.min()?o=n.min():o<t&&(o=t),o}async function Ci(e,t=1){const n=await e.run();l("LateStart");let o=t,r=n.pages;const s={title:i("STARTING"),html:h`
        ${i("CHOOSE_BEGINNING")}
      ...
      `,showCancelButton:!0,cancelButtonColor:"#d33",reverseButtons:!0,icon:"question",didOpen(){const c=document.querySelector("#pageBegin"),m=document.querySelector("#pageEnd"),g=rangeSlider(document.getElementById("pagesSlider"),{min:1,max:n.pages,value:[o,r],onInput(b,p){p&&([o,r]=b,c&&(c.value=o.toString()),m&&(m.value=r.toString()))}});function u(){if(c.value===""||m.value==="")return;const b=Oi(parseInt(c.value,10),r,g),p=Ii(parseInt(m.value,10),o,g);c.value=b.toString(),m.value=p.toString(),o=b,r=p,g.value([b,p])}const d=_.debounce(u,600);["change","mouseup","keyup","touchend"].forEach(b=>{c?.addEventListener(b,d),m?.addEventListener(b,d)})}};Swal.fire(s).then(c=>{c.value?(l(`Choice: ${o} - ${r}`),n.begin=o,n.pages=r,$(n).then(()=>l("Page loaded"))):l(c.dismiss)})}function St(e,t){const n=document.createElement("button");n.innerText=i("BUTTON_START"),n.id="StartMOV",n.onclick=()=>{Ci(e,t).catch(l)},document.body.appendChild(n);const o=document.createElement("style");o.appendChild(document.createTextNode(ui+tn)),document.head.appendChild(o),l("Start Button added to page",n)}function Li(e,t){Swal.fire({title:i("STARTING"),html:h`${t.begin>1?`${i("RESUME")}${t.begin}.<br/>`:""}${i("WAITING")}`,showCancelButton:!0,cancelButtonColor:"#d33",reverseButtons:!0,timer:3e3}).then(n=>{n.value||n.dismiss===Swal.DismissReason.timer?$(t).then(()=>l("Page loaded")):(St(e,t.begin),l(n.dismiss))})}async function Ai([e,t]){l(`Found Pages: ${t.pages} in ${e.name}`),t.title||(t.title=document.querySelector("title")?.textContent?.trim()),t.begin=R()??t.begin??1;const n=document.createElement("style");switch(n.appendChild(document.createTextNode(ze)),document.body.appendChild(n),unsafeWindow.MOV=(o,r)=>{o!==void 0&&(t.begin=o),r!==void 0&&(t.pages=r),$(t).then(()=>l("Page loaded"))},e.start??a()?.loadMode){case"never":St(e,t.begin);break;case"always":$(t).then(()=>l("Page loaded"));break;case"wait":default:Li(e,t);break}}async function Ti(e){if(l(`Starting ${oe.script.name} ${oe.script.version} on ${L()} ${dn()} with ${mn()}`),Mi())return;l(e.length,"Known Manga Sites:",e);const t=e.filter(o=>o.url.test(window.location.href));l(t.length,"Found sites:",t);const n=t.map(async o=>(l(`Testing site: ${o.name}`),new Promise((r,s)=>{Promise.all([bi(o),pi(o),gi(o),hi(o),wi(o)]).then(async()=>o.run()).then(c=>c.pages>0?r([o,c]):s(new Error(`${o.name} found ${c.pages} pages`)))})));Promise.race(n.map((o,r)=>o.then(()=>r))).then(o=>{n.forEach((r,s)=>{s!==o&&l(`Failed/Skipped: ${t[s].name}`)}),n[o].then(r=>{Ai(r)})})}Ti(en).catch(l)})();
